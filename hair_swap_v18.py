...
from models.Blending_v18 import Blending_v18
...
class HairFast_v18:
    """
    HairFast v18:
    author alignment -> SATD cleanup -> baseline-style color transfer.
    """
    ...
    def __init__(self, args):
        ...
        self.align = Alignment_v18(args, self.embed.get_e4e_embed, net=self.net)
        self.blend = Blending_v18(args, net=self.net, embed=self.embed, align=self.align)
...
def get_parser_v18():
    parser = argparse.ArgumentParser(description="HairFast v18")
    ...
    parser.add_argument("--blending_checkpoint_v18", type=str, default="pretrained_models/Blending/checkpoint.pth")
    parser.add_argument("--pp_checkpoint", type=str, default="pretrained_models/PostProcess/pp_model.pth")
    parser.add_argument("--clip_model_v18", type=str, default="ViT-B/32")
    parser.add_argument("--use_satd_v18", action="store_true")
    parser.add_argument("--satd_blend_v18", type=float, default=0.28)
    parser.add_argument("--use_post_satd_v18", action="store_true")
    parser.add_argument("--satd_checkpoint_v18", type=str, default="")
    parser.add_argument("--post_satd_blend_v18", type=float, default=0.28)
    parser.add_argument("--satd_boundary_v18", type=int, default=8)
    parser.add_argument("--eq8_reference_blend_v18", type=float, default=0.0)
    parser.add_argument(
        "--use_postprocess_v18",
        action="store_true",
        help="Run PostProcess after color transfer. Default is off to keep the v8-style SATD -> color path easy to inspect.",
    )
    parser.add_argument(
        "--postprocess_face_source_v18",
        action="store_true",
        help="Use the original face image as post-process source. Default keeps the color-blended image as source to avoid washing out transferred hair color.",
    )
    return parser


if __name__ == "__main__":
    args = get_parser_v18().parse_args()
    HairFast_v18(args)
