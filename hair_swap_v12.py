...
class HairFast_v12:
    """
    HairFast v12 inference entry point.

    v12 applies an optional topology-guided F32 shape adapter in Alignment_v12,
    then uses baseline-style blending with no F64 injection.
    """
    ...
    def __init__(self, args):
        ...
        self.align = Alignment_v12(args, self.embed.get_e4e_embed, net=self.net)
        self.blend = Blending_v12(args, net=self.net)
    ...
    def __swap_from_tensors(self, face: torch.Tensor, shape: torch.Tensor, color: torch.Tensor, **kwargs) -> torch.Tensor:
        ...
        if isinstance(alpha_mattes, dict):
            for name, alpha in alpha_mattes.items():
                if name in name_to_embed:
                    name_to_embed[name]["alpha_256"] = alpha.to(self.args.device)

        align_shape = self.align.align_images("face", "shape", name_to_embed, **kwargs)
        if shape is not color:
            align_color = self.align.shape_module("face", "color", name_to_embed, only_target=True, **kwargs)
        else:
...
def get_parser_v12():
    parser = argparse.ArgumentParser(description="HairFast v12")
    ...
    parser.add_argument("--shape_adapter_v12_checkpoint", type=str, default="")
    parser.add_argument("--use_shape_adapter_v12", action="store_true", default=False)
    parser.add_argument("--disable_shape_adapter_v12", dest="use_shape_adapter_v12", action="store_false")
    parser.add_argument("--shape_adapter_strength_v12", type=float, default=1.0)
    parser.add_argument("--shape_prior_strength_v12", type=float, default=0.0)
    parser.add_argument("--shape_adapter_hidden_v12", type=int, default=512)
    parser.add_argument("--shape_adapter_channels_v12", type=int, default=512)
    parser.add_argument("--topology_boundary_width_v12", type=int, default=9)
    parser.add_argument("--alpha_fallback_blur_v12", type=int, default=9)
    parser.add_argument("--bang_top_ratio_v12", type=float, default=0.56)
...
if __name__ == "__main__":
    args = get_parser_v12().parse_args()
    hair_fast = HairFast_v12(args)
