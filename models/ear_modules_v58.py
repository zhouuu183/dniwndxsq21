...
class EarAnchoredQueryBuilder(nn.Module):
    ...
    def forward(
        ...
        def build_earring_channel(
            ...
            channel = lobe_anchor
            for down in (8, 18, self.earring_channel_down):
                channel = torch.clamp(channel + shift_mask(lobe_anchor, down=down), 0, 1)
            channel = dilate_mask(channel, 3) * side_roi * side_visible
