...
class MaskEncoder_v16(nn.Module):
    def __init__(self, in_ch: int = 7, mid_ch: int = 64):
        super().__init__()
        self.net = nn.Sequential(
            ConvGNAct_v16(in_ch, 32),
            ConvGNAct_v16(32, 32),
            ConvGNAct_v16(32, mid_ch),
            ConvGNAct_v16(mid_ch, mid_ch),
        )
...
class FeatureProjector_v16(nn.Module):
...
class GateResidualHead_v16(nn.Module):
    def __init__(self, in_ch: int, feat_ch: int = 512):
        super().__init__()
        self.body = nn.Sequential(
            ConvGNAct_v16(in_ch, 256),
            ConvGNAct_v16(256, 128),
        )
...
class GateHead_v16(nn.Module):
    def __init__(self, in_ch: int):
        super().__init__()
        self.net = nn.Sequential(
            ConvGNAct_v16(in_ch, 128),
            ConvGNAct_v16(128, 64),
...
class ShadowContextHead_v16(nn.Module):
    def __init__(self, in_ch: int):
        super().__init__()
        self.net = nn.Sequential(
            ConvGNAct_v16(in_ch, 128),
...
class SATD_v16(nn.Module):
    """
    SATD cleanup branch used by SG-IDCT v16.
    ...
    def __init__(self, feat_ch: int = 512, proj_ch: int = 128, mask_ch: int = 7):
        super().__init__()
        self.mask_encoder = MaskEncoder_v16(mask_ch, 64)
        self.projector = FeatureProjector_v16(feat_ch, proj_ch)
        self.shadow_context = ShadowContextHead_v16(proj_ch * 3 + 64 + 3)
        self.halo_head = GateResidualHead_v16(proj_ch * 2 + 64 + 1, feat_ch=feat_ch)
        self.tail_head = GateResidualHead_v16(proj_ch * 3 + 64 + 1, feat_ch=feat_ch)
        self.neck_head = GateResidualHead_v16(proj_ch * 2 + 64 + 1, feat_ch=feat_ch)
        self.copy_head = GateResidualHead_v16(proj_ch * 2 + 64 + 1, feat_ch=feat_ch)
        self.protect_head = GateHead_v16(proj_ch * 2 + 64)
    ...
    def forward(
        ...
        shadow = self.shadow_context(torch.cat([P_base, P_src, P_src_inpaint, mask_feat, src_rgb_32], dim=1))
        ...
        protect_region = (body_preserve + 0.30 * remove_face * detail_map + 0.06 * remove_neck * detail_map).clamp(0, 1)

        halo_gate, halo_residual = self.halo_head(torch.cat([P_base, P_src_inpaint, mask_feat, shadow], dim=1))
        tail_gate, tail_residual = self.tail_head(torch.cat([P_base, P_src, P_src_inpaint, mask_feat, shadow], dim=1))
        neck_gate, neck_residual = self.neck_head(torch.cat([P_base, P_src_inpaint, mask_feat, shadow], dim=1))
        copy_gate, copy_residual = self.copy_head(torch.cat([P_base, P_src, mask_feat, shadow], dim=1))
