...
class PostProcessModelV52(nn.Module):
    def __init__(self, args: argparse.Namespace | None = None):
        ...
        self.parsing_helper = FaceParsingHelperV52(parse_size=getattr(self.args, "ear_parse_size", 512))
    ...
    def load_base_checkpoint(self, checkpoint_path: str | None):
        ...
        if relevant_missing:
            print(f"[PostProcessModelV52] Missing base keys: {len(relevant_missing)}")
            print(relevant_missing[:20])
        if result.unexpected_keys:
            print(f"[PostProcessModelV52] Unexpected checkpoint keys: {len(result.unexpected_keys)}")
    ...
    def _apply_direct_earring_restore(
        ...
        mask = aux.get("source_earring_clean_mask", aux.get("earring_write_mask", aux.get("source_earring_mask")))
