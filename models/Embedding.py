...
class Embedding(nn.Module):
    ...
    def embedding_images(self, images_to_name: dict[torch.Tensor, list[str]], **kwargs) -> dict[
        ...
        for image, names in self.dataloader:
            ...
            # V2.35 color disentanglement: the color branch must never encode
            # face/background pixels. Segmentation is computed from the
            # original image, then the independent color input is neutral-filled
            # outside its hair mask before E4E/FS encoding.
            masks = torch.cat([get_segmentation(image.unsqueeze(0)) for image in self.to_bisenet(im_512)])
            encoder_image = image.clone()
            if getattr(self.opts, "v235_enabled", False):
                for index, role_names in enumerate(names):
                    if "color" in role_names and all(role == "color" for role in role_names):
                        hair = (masks[index:index + 1] == 13).float()
                        hair = F.interpolate(hair, size=image.shape[-2:], mode="nearest")
                        encoder_image[index:index + 1] = (
                            image[index:index + 1] * hair
                            + 0.5 * (1.0 - hair)
                        )
            im_256 = self.downsample_256(encoder_image)
            ...
            # FS encoder
            output = self.encoder.test(img=self.normalize(encoder_image), return_latent=True)
