"""Deterministic full-color tone projector for Blending V8 v2.23."""
...
FULL_COLOR_ARCH_V8_6 = "full_color_tone_selective_projector_v823"
...
class FullColorToneSelectiveProjectorV823(nn.Module):
    ...
    def forward(
        ...
        *,
        ...
        hair_core = (
            masks["color_supervision_mask"] * (1.0 - hard_protect)
        ).clamp(0, 1)
        ...
        allowed_edge_l = (
            torch.maximum(base_l, pseudo_lab[:, :1]) + self.edge_luma_margin
        )
        edge_luma_excess_before_cap = torch.relu(l_out - allowed_edge_l) * hair_edge
        l_out = torch.where(
            hair_edge > 1e-6, torch.minimum(l_out, allowed_edge_l), l_out
        )

        candidate_rgb = lab_to_rgb(torch.cat((l_out, ab_out), dim=1))
        selective_rgb = (
            hair_support * candidate_rgb + (1.0 - hair_support) * base_rgb
        ).clamp(0, 1)
        selective_rgb = (
        ...
        aux = {
            ...
            "overshoot_fraction": _masked_mean(
                (parallel_mag > parallel_cap).float(), hair_support
            ),
