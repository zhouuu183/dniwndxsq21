...
class MaskEncoder_v8(nn.Module):
    def __init__(self, in_ch: int = 7, mid_ch: int = 64):
        super().__init__()
        self.net = nn.Sequential(
            ConvGNAct_v8(in_ch, 32),
            ConvGNAct_v8(32, 32),
            ConvGNAct_v8(32, mid_ch),
            ConvGNAct_v8(mid_ch, mid_ch),
        )
...
class FeatureProjector_v8(nn.Module):
...
class GateResidualHead_v8(nn.Module):
    def __init__(self, in_ch: int, feat_ch: int = 512):
        super().__init__()
        self.body = nn.Sequential(
            ConvGNAct_v8(in_ch, 256),
            ConvGNAct_v8(256, 128),
        )
...
class GateHead_v8(nn.Module):
    def __init__(self, in_ch: int):
        super().__init__()
        self.net = nn.Sequential(
            ConvGNAct_v8(in_ch, 128),
            ConvGNAct_v8(128, 64),
...
class ShadowContextHead_v8(nn.Module):
    def __init__(self, in_ch: int):
        super().__init__()
        self.net = nn.Sequential(
            ConvGNAct_v8(in_ch, 128),
...
class SATD_v8(nn.Module):
    """
    SATD-v5 implemented in the v8 branch.
    ...
    def __init__(self, feat_ch: int = 512, proj_ch: int = 128, mask_ch: int = 7):
        super().__init__()
        self.mask_encoder = MaskEncoder_v8(mask_ch, 64)
        self.projector = FeatureProjector_v8(feat_ch, proj_ch)
        self.shadow_context = ShadowContextHead_v8(proj_ch * 3 + 64 + 3)
        self.halo_head = GateResidualHead_v8(proj_ch * 2 + 64 + 1, feat_ch=feat_ch)
        self.tail_head = GateResidualHead_v8(proj_ch * 3 + 64 + 1, feat_ch=feat_ch)
        self.neck_head = GateResidualHead_v8(proj_ch * 2 + 64 + 1, feat_ch=feat_ch)
        self.copy_head = GateResidualHead_v8(proj_ch * 2 + 64 + 1, feat_ch=feat_ch)
        self.protect_head = GateHead_v8(proj_ch * 2 + 64)
    ...
    def forward(
        ...
        face_residue_region = (remove_face * (1.0 - 0.35 * detail_map)).clamp(0, 1)
        halo_region = (remove_halo + 0.18 * remove + 0.22 * remove_face + 0.20 * remove_neck).clamp(0, 1)
        tail_region = (1.08 * remove_tail + 0.42 * remove_halo + 0.42 * remove_neck + 0.16 * remove).clamp(0, 1)
        neck_region = (1.05 * remove_neck + 0.92 * remove_tail + 0.28 * remove_halo).clamp(0, 1)
        copy_region = (
        ...
        protect_region = (
            body_preserve
            + 0.48 * remove_face * detail_map
            + 0.08 * remove_neck * detail_map
        ).clamp(0, 1)
        ...
        face_residue_prior = 0.16 * face_residue_region * (F_src_inpaint - F_base)
        halo_prior = 0.14 * halo_region * (F_src_inpaint - F_base)
        tail_target = 0.70 * F_src_inpaint + 0.30 * F_src
        tail_prior = 0.34 * tail_region * (tail_target - F_base)
        neck_prior = 0.32 * neck_region * (F_src_inpaint - F_base)
        copy_prior = 0.04 * copy_region * (F_src - F_base)

        cleanup_delta = shadow_prior + face_residue_prior
        cleanup_delta = cleanup_delta + 0.11 * halo_gate * halo_region * (halo_prior + halo_residual)
        cleanup_delta = cleanup_delta + 0.23 * tail_gate * tail_region * (tail_prior + tail_residual)
        cleanup_delta = cleanup_delta + 0.22 * neck_gate * neck_region * (neck_prior + neck_residual)
        ...
        return output, {
            ...
            "face_residue_region": face_residue_region,
