...
def build_weak_earring_masks(
    ...
    candidate_local_area = F.avg_pool2d(earring_candidate_mask, kernel_size=5, stride=1, padding=2) * 25.0
    structured_candidate = (candidate_local_area >= 2.5).float()
    parser_support = dilate_mask(source_earring_clean_mask, 5)
    earring_candidate_mask = earring_candidate_mask * torch.clamp(structured_candidate + parser_support, 0, 1)
...
class EarAnchoredQueryBuilder(nn.Module):
    def __init__(
        ...
        target_ear_cover_overlap: float = 0.55,
        ...
        self.target_ear_cover_overlap = target_ear_cover_overlap
    ...
    def forward(
        ...
        target_open_context = (1 - target_hair_context).clamp(0, 1)
        left_target_visible = (target_masks["left_ear"] + target_left_ring) * target_open_context
        right_target_visible = (target_masks["right_ear"] + target_right_ring) * target_open_context
        ...
        left_target_visible = left_target_visible * target_open_context
        right_target_visible = right_target_visible * target_open_context
        ...
        left_target_visible_overlap = self._roi_overlap(left_target_visible, left_roi)
        right_target_visible_overlap = self._roi_overlap(right_target_visible, right_roi)
        left_side_covered = (
            (left_hair_overlap >= self.target_ear_cover_overlap)
            & (left_target_visible_overlap < self.min_target_visible_overlap)
        ).float()
        right_side_covered = (
            (right_hair_overlap >= self.target_ear_cover_overlap)
            & (right_target_visible_overlap < self.min_target_visible_overlap)
        ).float()

        left_side_visible = (left_hair_overlap < self.max_target_hair_overlap).float() * (1 - left_side_covered)
        right_side_visible = (right_hair_overlap < self.max_target_hair_overlap).float() * (1 - right_side_covered)
        left_side_visible_map = left_side_visible.view(-1, 1, 1, 1)
        right_side_visible_map = right_side_visible.view(-1, 1, 1, 1)
        left_side_covered_map = left_side_covered.view(-1, 1, 1, 1)
        right_side_covered_map = right_side_covered.view(-1, 1, 1, 1)

        left_open_region = left_roi * target_open_context
        right_open_region = right_roi * target_open_context
        target_visible_expand = max(3, self.ear_dilate // 2)
        left_visibility_anchor = dilate_mask(left_target_visible, target_visible_expand) * left_roi
        right_visibility_anchor = dilate_mask(right_target_visible, target_visible_expand) * right_roi
        left_visible_roi = (
            torch.clamp(left_target_visible + left_open_region * left_visibility_anchor, 0, 1)
            * left_side_visible_map
        )
        right_visible_roi = (
            torch.clamp(right_target_visible + right_open_region * right_visibility_anchor, 0, 1)
            * right_side_visible_map
        )
        visible_ear_roi = torch.clamp(left_visible_roi + right_visible_roi, 0, 1)
        target_covered_ear_block_mask = torch.clamp(
            left_roi * left_side_covered_map + right_roi * right_side_covered_map,
            0,
            1,
        )
        ...
        return {
            ...
            "target_covered_ear_block_mask": target_covered_ear_block_mask,
