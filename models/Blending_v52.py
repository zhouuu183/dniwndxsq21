...
class BlendingV52(nn.Module):
    """
    V8-cleaned blending + V52 refinement entry.
    ...
    ear/detail-aware PostProcessModelV52.
    """
    ...
    def __init__(self, opts, net=None):
        ...
        pp_args = argparse.Namespace(
            use_mod=getattr(self.opts, "pp_v52_use_mod", True),
            use_full=getattr(self.opts, "pp_v52_use_full", True),
            pretrain=False,
            finetune=False,
        ...
        self.post_process = PostProcessModelV52(pp_args).to(self.opts.device).eval()

        checkpoint_path = getattr(self.opts, "pp_v52_checkpoint", None) or getattr(self.opts, "pp_checkpoint", None)
