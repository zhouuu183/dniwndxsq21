...
from models.SATD_v16 import SATD_v16
from models.sean_codes.models.pix2pix_model import decode_sean, encode_sean
from utils.mask_delta_v16 import (
    build_delta_masks,
    drop_parsing_labels,
    enrich_delta_masks_with_halo,
    filter_parsing_to_primary_subject,
    stack_cleanup_masks_v16,
)


class Alignment_v16(Alignment):
    """
    SG-IDCT v16 alignment.

    The class keeps the author alignment as the structure prior and applies the
    SATD cleanup branch only to the aligned feature map. It also exposes the
    delta masks needed by the image-space color transfer module.
    """

    def __init__(self, opts, latent_encoder=None, net=None, satd_model_v16: SATD_v16 | None = None):
        super().__init__(opts, latent_encoder=latent_encoder, net=net)
        self.satd_model_v16 = satd_model_v16

        if self.satd_model_v16 is None and getattr(self.opts, "use_satd_v16", False):
            self.satd_model_v16 = SATD_v16().to(self.opts.device).eval()
            ckpt_path = getattr(self.opts, "satd_checkpoint_v16", "")
            if ckpt_path:
                ckpt = torch.load(ckpt_path, map_location=self.opts.device)
                state_dict = ckpt.get(
                    "satd_v16_state_dict",
                    ckpt.get("satd_v8_state_dict", ckpt.get("model_state_dict", ckpt)),
                )
                self._load_compatible_state_dict(self.satd_model_v16, state_dict)
    ...
    @staticmethod
    def _ensure_image_batch_v16(image: torch.Tensor) -> torch.Tensor:
        if image.dim() == 3:
    ...
    def shape_module(self, im_name1: str, im_name2: str, name_to_embed, only_target=True, **kwargs):
        ...
        hair_mask1 = torch.where(
            inp_mask1 == 13,
            torch.ones_like(inp_mask1, device=device),
            torch.zeros_like(inp_mask1, device=device),
        ).float()
        hair_mask2 = torch.where(
            inp_mask2 == 13,
            torch.ones_like(inp_mask2, device=device),
    ...
    def author_align_images(self, im_name1, im_name2, name_to_embed, **kwargs):
        ...
        gen1_sean = self._ensure_image_batch_v16(decode_sean(self.sean_model, img1_code.unsqueeze(0), target_mask))
        gen2_sean = self._ensure_image_batch_v16(decode_sean(self.sean_model, img2_code.unsqueeze(0), target_mask))
        ...
        latent_F_align = self._eq16_fusion(
    ...
    @staticmethod
    def _reference_enhance_eq16(
        latent_F_eq16: torch.Tensor,
        ...
        return latent_F_eq16 + blend * target_region * (reference_anchor - latent_F_eq16)
    ...
    def prepare_satd_features(self, im_name1: str, im_name2: str, name_to_embed, **kwargs):
        ...
        latent_F_1 = name_to_embed[im_name1]["F"]
        latent_F_2 = name_to_embed[im_name2]["F"]
        boundary_width = kwargs.get("satd_boundary_v16", getattr(self.opts, "satd_boundary_v16", self.opts.smooth))
        ...
        if img1_in is img2_in:
            ...
            cleanup_masks_256 = stack_cleanup_masks_v16(delta_masks)
            img1_norm = img1_in * 2 - 1
            return {
                "latent_F_eq16": latent_F_1,
        ...
        img1_code, img2_code = encode_sean(self.sean_model, images, labels)
        gen1_sean = self._ensure_image_batch_v16(decode_sean(self.sean_model, img1_code.unsqueeze(0), target_mask_clean))
        gen2_sean = self._ensure_image_batch_v16(decode_sean(self.sean_model, img2_code.unsqueeze(0), target_mask_clean))

        enc_imgs = self.latent_encoder([gen1_sean[0], gen2_sean[0]])
        intermediate_align = enc_imgs["F"][0].unsqueeze(0)
        latent_F_out_new = enc_imgs["F"][1].unsqueeze(0)

        latent_F_eq16 = self._eq16_fusion(
            latent_F_1=latent_F_1,
            latent_F_2=latent_F_2,
            intermediate_align=intermediate_align,
            latent_F_out_new=latent_F_out_new,
        ...
        cleanup_masks_256 = stack_cleanup_masks_v16(delta_masks)
        latent_F_base = self._reference_enhance_eq16(
            latent_F_eq16=latent_F_eq16,
            latent_F_ref=latent_F_2,
            latent_F_shape_inpaint=latent_F_out_new,
            delta_masks=delta_masks,
            blend=kwargs.get(
                "eq16_reference_blend_v16",
                getattr(self.opts, "eq16_reference_blend_v16", 0.0),
            ),
        )
    ...
    def align_images(self, im_name1, im_name2, name_to_embed, **kwargs):
        ...
        use_satd = kwargs.get("use_satd_v16", getattr(self.opts, "use_satd_v16", False))
        if (
            use_satd
            and self.satd_model_v16 is not None
            and name_to_embed[im_name1]["image_256"] is not name_to_embed[im_name2]["image_256"]
        ):
            satd_out, _ = self.satd_model_v16(
            ...
            satd_blend = kwargs.get("satd_blend_v16", getattr(self.opts, "satd_blend_v16", 0.28))
