...
class PostProcessModelV5(nn.Module):
    ...
    def _enhance_query_recall(
        ...
        visible_mask = resize_mask(visible_ear_roi, query_mask.shape[-2:])
        ...
        recall_info["earring_highlight_mask"] = (
            resize_mask(weak_masks["earring_highlight_mask"], query_mask.shape[-2:]) * visible_ear_roi
        )
    ...
    def _resolve_masks(
        ...
        if source_ear_mask is not None:
            ...
            merge_aux_mask("source_earring_mask", gated_aux_mask(source_ear_mask, visible_ear_roi))
        if source_earring_object_mask is not None:
            ...
            merge_aux_mask("source_earring_mask", gated_aux_mask(source_earring_object_mask, visible_ear_roi))
            merge_aux_mask("source_earring_object_mask", gated_aux_mask(source_earring_object_mask, visible_ear_roi))
        if earring_confident_mask is not None:
            ...
            merge_aux_mask("source_earring_mask", gated_aux_mask(earring_confident_mask, visible_ear_roi))
            merge_aux_mask("earring_confident_mask", gated_aux_mask(earring_confident_mask, visible_ear_roi))
        if earring_highlight_mask is not None:
            merge_aux_mask("earring_highlight_mask", gated_aux_mask(earring_highlight_mask, visible_ear_roi))
        ...
        if recovery_seed.detach().flatten(1).amax(dim=1).max().item() > 0:
            ...
            if visible_ear_roi is not None:
                recovery_seed = recovery_seed * resize_mask(visible_ear_roi, recovery_seed.shape[-2:])
