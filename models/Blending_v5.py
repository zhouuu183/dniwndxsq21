...
class BlendingV5(Blending_v8):
    ...
    def __init__(self, opts, net=None):
        ...
        pp_args = argparse.Namespace(
            ...
            source_hair_face_suppress_dilate=pp_value("source_hair_face_suppress_dilate", 5),
            source_hair_face_suppress_strength=pp_value("source_hair_face_suppress_strength", 0.65),
            source_hair_face_suppress_max_y=pp_value("source_hair_face_suppress_max_y", 0.45),
            source_hair_face_suppress_ear_exclude_dilate=pp_value(
                "source_hair_face_suppress_ear_exclude_dilate",
                9,
            ),
            enable_output_target_preserve=pp_value("enable_output_target_preserve", True),
            output_target_hair_preserve_dilate=pp_value("output_target_hair_preserve_dilate", 5),
            output_face_hair_seam_preserve_dilate=pp_value(
                "output_face_hair_seam_preserve_dilate",
                7,
            ),
            output_earring_keep_dilate=pp_value("output_earring_keep_dilate", 0),
            output_preserve_blur=pp_value("output_preserve_blur", 1),
            output_hairline_feather=pp_value("output_hairline_feather", 9),
            # Must match training (pp_train_v5 sets this False): harmonize is a
            # self.training-gated inference-only low-freq diffuse fill.  If it were
            # on only at deployment, the revealed forehead would get an extra
            # smoothing the model never saw in training -> the plastic/airbrushed
            # look.  Keep it off so revealed skin stays the raw generator output.
            enable_revealed_skin_harmonize=pp_value("enable_revealed_skin_harmonize", False),
            revealed_skin_harmonize_strength=pp_value("revealed_skin_harmonize_strength", 0.9),
            revealed_skin_tone_kernel=pp_value("revealed_skin_tone_kernel", 21),
            revealed_skin_tone_sigma=pp_value("revealed_skin_tone_sigma", 7.0),
            revealed_skin_diffuse_iters=pp_value("revealed_skin_diffuse_iters", 24),
            revealed_skin_tone_limit=pp_value("revealed_skin_tone_limit", 0.35),
            revealed_skin_detail_gain=pp_value("revealed_skin_detail_gain", 1.15),
        )
