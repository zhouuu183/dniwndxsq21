...
class Blending_v19(nn.Module):
    """
    v19 blending that preserves boundary-band F64 detail from shape alignment.
    """
    ...
    def __init__(self, opts, net=None):
        ...
        self.final_detail_strength = float(getattr(self.opts, "shape_v19_final_detail_strength", 0.65))

    def _render_with_shape_detail(
        self,
        latent_s: torch.Tensor,
        latent_f32: torch.Tensor,
        latent_f64: torch.Tensor | None,
    ) -> torch.Tensor:
        if latent_f64 is None:
    ...
    def blend_images(self, align_shape, align_color, name_to_embed, **kwargs):
        ...
        latent_F64_detail = align_shape.get("latent_F64_detail")
        HM_X = align_color["HM_X"]
        latent_S_3 = name_to_embed["color"]["S"]
        boundary_band = align_shape.get("boundary_band")
        ...
        I_blend = self._render_with_shape_detail(S_blend, latent_F_align, latent_F64_detail)
        I_blend_256 = self.downsample_256(I_blend)

        S_final, F_final = self.post_process(I_1, I_blend_256)
        if latent_F64_detail is not None:
            if boundary_band is not None:
                boundary_band = F.interpolate(boundary_band.float(), size=F_final.shape[-2:], mode="bilinear", align_corners=False)
                F_final = F_final + self.final_detail_strength * boundary_band * (latent_F64_detail - F_final)
        I_final, _ = self.net.generator([S_final], input_is_latent=True, return_latents=False, start_layer=5, end_layer=8, layer_in=F_final)

        if self.opts.save_all:
            exp_name = kwargs.get("exp_name") or ""
            output_dir = self.opts.save_all_dir / exp_name
            save_gen_image(output_dir, "Blending_v19", "blending.png", I_blend)
            save_latents(output_dir, "Blending_v19", "blending.npz", S_blend=S_blend, latent_F_align=latent_F_align)
            save_gen_image(output_dir, "Final_v19", "final.png", I_final)
            save_latents(output_dir, "Final_v19", "final.npz", S_final=S_final, F_final=F_final)
