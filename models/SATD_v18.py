...
class MaskEncoder_v18(nn.Module):
    def __init__(self, in_ch: int = 7, mid_ch: int = 64):
        super().__init__()
        self.net = nn.Sequential(
            ConvGNAct_v18(in_ch, 32),
            ConvGNAct_v18(32, 32),
            ConvGNAct_v18(32, mid_ch),
            ConvGNAct_v18(mid_ch, mid_ch),
        )
...
class FeatureProjector_v18(nn.Module):
...
class GateResidualHead_v18(nn.Module):
    def __init__(self, in_ch: int, feat_ch: int = 512):
        super().__init__()
        self.body = nn.Sequential(
            ConvGNAct_v18(in_ch, 256),
            ConvGNAct_v18(256, 128),
        )
...
class GateHead_v18(nn.Module):
    def __init__(self, in_ch: int):
        super().__init__()
        self.net = nn.Sequential(
            ConvGNAct_v18(in_ch, 128),
            ConvGNAct_v18(128, 64),
...
class ShadowContextHead_v18(nn.Module):
    def __init__(self, in_ch: int):
        super().__init__()
        self.net = nn.Sequential(
            ConvGNAct_v18(in_ch, 128),
...
class SATD_v18(nn.Module):
    """
    Cleanup-only SATD branch used after color blending in v18.
    ...
    def __init__(self, feat_ch: int = 512, proj_ch: int = 128, mask_ch: int = 7):
        super().__init__()
        self.mask_encoder = MaskEncoder_v18(mask_ch, 64)
        self.projector = FeatureProjector_v18(feat_ch, proj_ch)
        self.shadow_context = ShadowContextHead_v18(proj_ch * 3 + 64 + 3)
        self.halo_head = GateResidualHead_v18(proj_ch * 2 + 64 + 1, feat_ch=feat_ch)
        self.tail_head = GateResidualHead_v18(proj_ch * 3 + 64 + 1, feat_ch=feat_ch)
        self.neck_head = GateResidualHead_v18(proj_ch * 2 + 64 + 1, feat_ch=feat_ch)
        self.copy_head = GateResidualHead_v18(proj_ch * 2 + 64 + 1, feat_ch=feat_ch)
        self.protect_head = GateHead_v18(proj_ch * 2 + 64)
