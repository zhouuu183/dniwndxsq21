...
from models.Alignment_v14 import AlignmentV14
from models.Blending_v14 import BlendingV14
...
TReturn = tp.TypeVar("TReturn", torch.Tensor, tuple[torch.Tensor, ...], dict)


class HairFastV14:
    """
    HairFast v14 with NHR-Branch inserted in shape alignment.
    """
    ...
    def __init__(self, args):
        ...
        self.align = AlignmentV14(args, self.embed.get_e4e_embed, net=self.net)
        self.blend = BlendingV14(args, net=self.net)
    ...
    def _apply_shadow_cleanup(self, face: torch.Tensor, final_image: torch.Tensor, name_to_embed, align_shape, **kwargs) -> torch.Tensor:
        use_cleanup = kwargs.get("use_shadow_cleanup", getattr(self.args, "use_shadow_cleanup", False))
        if not use_cleanup:
            return final_image
        if self.shadow_cleanup is None or _build_shadow_cleanup_masks_impl is None:
            return final_image
    ...
    def __swap_from_tensors(self, face: torch.Tensor, shape: torch.Tensor, color: torch.Tensor, **kwargs):
        ...
        return_pipeline_info = kwargs.get("return_pipeline_info", False)
        blend_result = self.blend.blend_images(
            align_shape,
            align_color,
            name_to_embed,
            return_pipeline_info=return_pipeline_info,
            **kwargs,
        )
        if return_pipeline_info:
            final_image = blend_result["final_image"]
        else:
            final_image = blend_result
        final_image = self._apply_shadow_cleanup(face, final_image, name_to_embed, align_shape, **kwargs)

        if return_pipeline_info:
            blend_result["final_image"] = final_image
            return {
    ...
    def swap(self, face_img: TImage | TPath, shape_img: TImage | TPath, color_img: TImage | TPath, benchmark=False, align=False, seed=None, exp_name=None, **kwargs) -> TReturn:
        ...
        result = self.__swap_from_tensors(*images, seed=seed, benchmark=benchmark, exp_name=exp_name, **kwargs)
        if align:
...
def get_parser_v14():
    parser = argparse.ArgumentParser(description="HairFast v14")
    ...
    parser.add_argument("--pp_v14_checkpoint", type=str, default="")
    parser.add_argument("--repair_checkpoint", type=str, default="")

    parser.add_argument("--use_nhr_branch", type=int, default=1)
    parser.add_argument("--nhr_input_channels", type=int, default=17)
    parser.add_argument("--nhr_base_channels", type=int, default=32)
    parser.add_argument("--nhr_occ_dilate", type=int, default=7)
    parser.add_argument("--nhr_ring_dilate", type=int, default=9)
    parser.add_argument("--nhr_ring_erode", type=int, default=1)
    parser.add_argument("--nhr_use_ring", type=int, default=1)
    parser.add_argument("--nhr_disable_local_bypass", type=int, default=0)
    parser.add_argument("--nhr_bypass_mode", type=str, default="gated_residual")
    parser.add_argument("--nhr_bypass_scale", type=float, default=0.15)
    parser.add_argument("--nhr_bg0_blur_kernel", type=int, default=11)
    parser.add_argument("--nhr_bg0_sean_blend", type=float, default=0.15)
    parser.add_argument("--nhr_use_raw_source_for_refine", type=int, default=0)
