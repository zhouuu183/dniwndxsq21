...
from models.Blending_v51 import BlendingV51
...
class HairFastV51:
    def __init__(self, args):
        ...
        if getattr(self.args, "use_satd_v8", False) and not getattr(self.args, "satd_checkpoint_v8", ""):
            print("[HairFastV51] use_satd_v8=True but satd_checkpoint_v8 is empty; disabling SATD_v8 cleanup.")
            self.args.use_satd_v8 = False
        self.net = Net(self.args)
        self.embed = Embedding(args, net=self.net)
        # v51 now uses the v8 shadow-cleaned alignment path before the
        # ear-aware post-process stage.
        self.align = Alignment_v8(args, self.embed.get_e4e_embed, net=self.net)
        self.blend = BlendingV51(args, net=self.net)
...
def get_parser():
    parser = argparse.ArgumentParser(description="HairFast V51")
    ...
    parser.add_argument("--pp_v51_checkpoint", type=str, default="pretrained_models/PostProcess/pp_model.pth")
    ...
    parser.add_argument("--pp_v51_use_mod", type=str2bool, default=True)
    parser.add_argument("--pp_v51_use_full", type=str2bool, default=True)
