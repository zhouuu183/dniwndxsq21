...
from hair_swap_v19 import HairFast_v19, get_parser
    ...
    "experiments": [
        {
            ...
            "shape_name": "7.png",
            "color_name": "8.png",
            "result_path": Path("output/result_v19_01.png"),
        },
    ],
    "save_all": True,
...
def _normalize_user_experiments():
    ...
    for idx, experiment in enumerate(experiments, start=1):
        if not isinstance(experiment, dict):
            raise TypeError(
                f"USER_CONFIG['experiments'][{idx - 1}] must be a dict. Different sample groups should be separated by commas."
            )
        ...
        if missing:
            raise ValueError(f"USER_CONFIG['experiments'][{idx - 1}] is missing values for: {missing}")
        normalized.append(
            {
                "face_path": face_path,
...
def main(model_args, args):
    hair_fast = HairFast_v19(model_args)
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="HairFast v19 evaluate")
    parser.add_argument("--input_dir", type=Path, default="", help="Directory of input images")
    parser.add_argument("--benchmark", action="store_true", help="Measure runtime during the session")
    parser.add_argument("--file_path", type=Path, default=None, help='File with experiments: "face shape color"')
    parser.add_argument("--output_dir", type=Path, default=Path("output"), help="Directory for final results")
