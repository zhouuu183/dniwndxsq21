"""Compute LPIPS and PSNR for a frozen 3000-image reconstruction set."""
...
USER_RESULT_DIR = Path("output/celeba_hq_baseline_recon/author_baseline/results")
USER_OUTPUT_CSV = Path("output/lpips_psnr_baseline_recon_3000.csv")
USER_IMAGE_SIZE = (256, 256)
USER_DEVICE = "cuda"
USER_CUDA_VISIBLE_DEVICES = "0"
USER_LPIPS_NET = "alex"
# LPIPS metric's official learned calibration weight. This is not a HairFast
# checkpoint. Download it once; every baseline/other-model comparison reuses it.
...
def load_manifest(path: Path) -> list[dict[str, Any]]:
    if not path.is_file():
        raise FileNotFoundError(
            f"Missing reconstruction manifest: {path}. Run scripts/make_recon_manifest.py first."
        )
