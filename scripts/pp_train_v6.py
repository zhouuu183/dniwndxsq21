...
# ``dataset_config.json`` was added after some otherwise usable V5 parts had
# already been generated.  These are the fields accessed unconditionally by
# ``PPDatasetV5.__getitem__`` (and its positive-sample indexer).  Optional
# earring supervision is deliberately not included: the loader provides safe
# zero/default fallbacks for it.
PP_DATASET_REQUIRED_ITEM_KEYS = (
    "source_path",
    "target",
...
# Native source detail is conditioned globally at the PP 128px decoder layer.
# It has no face/hair-region gate and never becomes final RGB compositing.
USER_ENABLE_NATIVE_SOURCE_DETAIL = False
USER_NATIVE_SOURCE_DETAIL_WORK_SIZE = 384
USER_NATIVE_SOURCE_DETAIL_CHANNELS = 64
USER_NATIVE_SOURCE_DETAIL_LOSS_SIZE = 384
USER_LAMBDA_NATIVE_SOURCE_DETAIL = 0.0
...
USER_LAMBDA_DETAIL_LOW_ANCHOR = 0.0
...
USER_LAMBDA_SOURCE_VALID_FACE_HIGH = 0.0
USER_LAMBDA_SOURCE_VALID_FACE_COLOR = 0.0
# The visible face is copied from the native source by the V19 compositor;
# the newly exposed region must be learned from one global PP condition.
# Region-specific face anchors, smoothness and texture statistics otherwise
# teach a separate low-frequency forehead state.
USER_LAMBDA_FACE_SOURCE_DETAIL_HR = 0.0
USER_LAMBDA_FACE_LOWFREQ_ANCHOR_HR = 0.0
USER_LAMBDA_FACE_LOWFREQ_CONTINUITY_HR = 0.0
USER_LAMBDA_REVEALED_BOUNDARY_SEAM_HR = 0.0
USER_LAMBDA_REVEALED_SOURCE_TEXTURE_STAT_HR = 0.0
USER_LAMBDA_FACE_HAIR_BOUNDARY_SEAM_HR = 0.0
USER_LAMBDA_FACE_LOWFREQ_CONTINUITY = 0.0
USER_LAMBDA_REVEALED_BOUNDARY_SEAM = 0.0
USER_LAMBDA_REVEALED_TEXTURE_STAT = 0.0
...
USER_LAMBDA_FACE_SOURCE_DARK_REJECT = 0.0
...
RESOLVED_USER_CONFIG = {
    ...
    "enable_native_source_detail": USER_ENABLE_NATIVE_SOURCE_DETAIL,
    "native_source_detail_work_size": USER_NATIVE_SOURCE_DETAIL_WORK_SIZE,
    "native_source_detail_channels": USER_NATIVE_SOURCE_DETAIL_CHANNELS,
    "native_source_detail_loss_size": USER_NATIVE_SOURCE_DETAIL_LOSS_SIZE,
    "native_source_detail": USER_LAMBDA_NATIVE_SOURCE_DETAIL,
    ...
    "satd_background_target_hair_protect_dilate": USER_SATD_BACKGROUND_TARGET_HAIR_PROTECT_DILATE,
...
def build_parser(defaults):
    parser = argparse.ArgumentParser(description="Post Process trainer v6")
    ...
    parser.add_argument(
        "--enable_native_source_detail",
        type=str2bool,
        default=defaults["enable_native_source_detail"],
    )
    parser.add_argument(
        "--native_source_detail_work_size",
        type=int,
        default=defaults["native_source_detail_work_size"],
    )
    parser.add_argument(
        "--native_source_detail_channels",
        type=int,
        default=defaults["native_source_detail_channels"],
    )
    parser.add_argument(
        "--native_source_detail_loss_size",
        type=int,
        default=defaults["native_source_detail_loss_size"],
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        type=int,
        default=defaults["satd_background_target_hair_protect_dilate"],
        help="Protect this dilated author target-hair support before SATD removes ghost hair residue.",
    )
    ...
    parser.add_argument(
        "--native_source_detail",
        type=float,
        default=defaults["native_source_detail"],
    )
...
def configure_training_stage(model: PostProcessModelV6, args):
    ...
    if args.training_stage == "ear_only":
        modules = [
            ...
            model.native_source_detail_conditioner,
            model.native_source_detail_injector_128,
        ]
    elif args.training_stage == "joint_highres":
        # ``joint_highres`` keeps its public training mode, but the author PP
        # face core is deliberately excluded.  This run has only 30 samples;
        # allowing target-preservation and earring losses to update
        # ``to_feature`` is what previously converted the pretrained global
        # face decoder into a low-frequency, full-face output.
        modules = [
            model.hf_extractor,
            model.mask_refresher,
            model.brightness_reestimator,
...
class TrainerV5:
    def __init__(
        ...
        if args is not None:
            loss_weights = {
                ...
                "native_source_detail": args.native_source_detail,
                "native_source_detail_loss_size": args.native_source_detail_loss_size,
    ...
    def build_satd_debug_row(self, aux, idx):
        """Return native-resolution SATD before/after/mask/delta panels."""
        before = aux.get("satd_pp_before_01")
        after = aux.get("satd_pp_after_01")
        alpha = aux.get("satd_background_cleanup_alpha")
        if not torch.is_tensor(before) or not torch.is_tensor(after):
            return []
        before_image = before[idx].detach().cpu().clamp(0, 1)
        after_image = after[idx].detach().cpu().clamp(0, 1)
        size = tuple(before_image.shape[-2:])
        alpha_image = self.mask_to_rgb(alpha[idx] if torch.is_tensor(alpha) else None, size)
        delta_image = ((after_image - before_image).abs() * 8.0).clamp(0, 1)
        return [before_image, after_image, alpha_image, delta_image]
    ...
    def _run_model(self, batch):
        ...
        source_native_earring_alpha = batch.get("source_native_earring_alpha")
        source_ear_mask = (
            source_native_earring_alpha
            if use_dataset_source_mask and torch.is_tensor(source_native_earring_alpha)
            else None
        )
        source_earring_object_mask = source_ear_mask
        # ``dataset_instance_mask`` is TARGET_CANONICAL supervision.  It is
        # still passed to the learned earring loss below, but must never be
        # reused as SOURCE_NATIVE alpha for RGB extraction/compositing.
        ...
        if torch.is_tensor(dataset_instance_mask) and torch.is_tensor(dataset_instance_gate):
            ...
            #
            # Do not write ``exact_instance`` into the two source-native
            # fields below.  ``earring_learning_mask`` is TARGET_CANONICAL;
            # using it as RGB extraction alpha shifts the source pixels into
            # the wrong coordinate system and brings back source background
            # strands or floating earrings.  The source-native alpha was
            # supplied before the model call and is restored immediately
            # after the supervision merge below.  ``source_earring_mask`` is
            # still a 256px loss/query mask; only the object field is reset to
            # native resolution for the final RGB compositor.
        ...
        # Reassert this outside the learned-mask gate as well: a zero native
        # alpha is an intentional no-earring sample, and must not fall back to
        ...
        # The active PP model receives exactly the author's two PP inputs and
        # makes the only S/F pair used by the StyleGAN decode. SATD and the
        # source-native earring compositor run strictly after this image.
        gen_im_F, aux = self.model.render_refined(
            self.net.generator,
            latent_s,
            latent_f_gen,
            aux,
    ...
    def train_one_epoch(self):
        ...
        if self.args.training_stage == "joint_highres":
            # ``requires_grad=False`` does not stop BatchNorm-like modules
            # from updating state in train mode.  Preserve the exact loaded
            # author PP core throughout the earring training run.
            self.model.lock_baseline_face_core_eval()
    ...
    def validate(self, epoch_tag="initial"):
        ...
        satd_stat_totals = {
            "candidate_delta": 0.0,
            "cleanup_alpha_area": 0.0,
            "cleanup_residual": 0.0,
            "applied_delta": 0.0,
        }
        satd_stat_count = 0
        ...
        for batch in tqdm(self.test_dataloader):
            ...
            satd_stat_fields = {
                "candidate_delta": "satd_background_candidate_delta_mean",
                "cleanup_alpha_area": "satd_background_cleanup_alpha_area",
                "cleanup_residual": "satd_background_cleanup_residual_mean",
                "applied_delta": "satd_background_applied_delta_mean",
            }
            for stat_name, aux_name in satd_stat_fields.items():
                value = aux.get(aux_name)
                if torch.is_tensor(value):
                    satd_stat_totals[stat_name] += float(value.detach().sum().item())
            satd_stat_count += source.size(0)
        ...
        missing_preview_count = preview_count - len(preview_items)
        train_preview_items = []
        if (
            ...
            for batch in tqdm(self.train_preview_dataloader, desc="Supplement train previews", leave=False):
                ...
                take_count = min(source.size(0), missing_preview_count - len(train_preview_items))
            ...
            preview_items.extend(train_preview_items)
        ...
        if preview_items and len(preview_items) < preview_count:
            print(
                f"Validation preview request ({preview_count}) exceeds the number of available distinct samples; "
                f"saved {len(preview_items)} distinct preview(s) without reuse."
            )
        ...
        if satd_stat_count:
            satd_stats = {
                key: value / satd_stat_count
                for key, value in satd_stat_totals.items()
            }
            self.logger.log_scalars(
                {f"val satd {key}": value for key, value in satd_stats.items()}
            )
            print(
                "SATD validation: "
                f"candidate_delta={satd_stats['candidate_delta']:.6f}, "
                f"cleanup_alpha_area={satd_stats['cleanup_alpha_area']:.1f}, "
                f"cleanup_residual={satd_stats['cleanup_residual']:.6f}, "
                f"applied_delta={satd_stats['applied_delta']:.6f}"
            )

        if preview_items:
            preview_files = [item[0] for item in preview_items]
            highres_preview_files = [item[1] for item in preview_items]
...
def load_dataset_part(path):
    """Load either legacy raw parts or new lossless-gzip V6 parts.

    The extension remains ``.dataset`` so existing dataset discovery works.
    Inspecting the two-byte magic instead of trusting metadata keeps old,
    already-complete V6 data readable as well.
    """

    path = Path(path)
    with path.open("rb") as probe:
        gzip_encoded = probe.read(2) == b"\x1f\x8b"
    if gzip_encoded:
...
class PPDatasetV5(Dataset):
    ...
    def load_part_items(self, part_path):
        ...
        if self._cached_part_path != part_path:
            ...
            self._cached_part_path = None
            gc.collect()
            self._cached_part_items = load_dataset_part(part_path)
    ...
    def __getitem__(self, idx):
        ...
        sample = {
            ...
            "source_native_earring_alpha": item.get(
                "source_native_earring_alpha",
...
def build_dataset_index(dataset_dir: Path, query_area_threshold: float):
    ...
    metadata_missing = not config_path.is_file()
    if not metadata_missing:
        ...
        print(
            f"PP dataset metadata is missing ({config_path}); validating existing parts "
            "against the current V5 loader contract."
        )
    ...
    for file in tqdm(files, desc="Index dataset parts"):
        part_items = load_dataset_part(file)
        if not isinstance(part_items, (list, tuple)):
            compatibility_errors.append(
                f"{file}: expected a list of dataset items, got {type(part_items).__name__}"
            )
            del part_items
            gc.collect()
            continue
        part_lengths.append(len(part_items))
        for item_idx, item in enumerate(part_items):
            if metadata_missing:
                if not isinstance(item, dict):
                    compatibility_errors.append(
                        f"{file} item {item_idx}: expected dict, got {type(item).__name__}"
                    )
                    continue
                missing_keys = [
                    key for key in PP_DATASET_REQUIRED_ITEM_KEYS if key not in item
                ]
                if missing_keys:
                    compatibility_errors.append(
                        f"{file} item {item_idx}: missing {', '.join(missing_keys)}"
                    )
                    continue
    ...
    if compatibility_errors:
        details = "\n  ".join(compatibility_errors[:20])
        remaining = len(compatibility_errors) - 20
        suffix = f"\n  ... and {remaining} more incompatible item(s)" if remaining > 0 else ""
        raise RuntimeError(
            "PP dataset metadata is missing and its existing parts do not satisfy "
            "the current V5 loader contract. Regenerate the complete V5 dataset.\n"
            f"  {details}{suffix}"
        )
    if metadata_missing:
        print(
            "PP dataset metadata compatibility check passed; training will use the "
            "existing complete parts without regenerating them."
        )
