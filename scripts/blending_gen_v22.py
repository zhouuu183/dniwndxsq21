...
from models.ColorTransfer_v22 import build_v22_lock_mask
...
USER_OUTPUT_DIR_FFHQ = Path("input/blending_dataset_v22")
...
USER_OUTPUT_DIR_SMALL = Path("input/blending_dataset_v22_small")
...
def load_rgb_256(path: Path) -> torch.Tensor:
    with Image.open(path) as image:
        tensor = T.functional.to_tensor(image.convert("RGB"))
    if tuple(tensor.shape[-2:]) != (USER_INPUT_TRIPLET_SIZE, USER_INPUT_TRIPLET_SIZE):
...
def save_panel(path: Path, face_path: Path, shape_path: Path, color_path: Path, satd_01: torch.Tensor) -> None:
    ...
    face = load_rgb_256(face_path)
    shape = load_rgb_256(shape_path)
    color = load_rgb_256(color_path)
    if tuple(satd_01.shape[-2:]) != (USER_INPUT_TRIPLET_SIZE, USER_INPUT_TRIPLET_SIZE):
        satd_01 = F.interpolate(
            satd_01.unsqueeze(0),
            size=(USER_INPUT_TRIPLET_SIZE, USER_INPUT_TRIPLET_SIZE),
            mode="bilinear",
            align_corners=False,
        )[0]
    save_image(torch.cat([face, shape, color, satd_01], dim=2), path)
...
def main() -> None:
    ...
    satd_dir = ACTIVE_OUTPUT_DIR / "V22"
    ...
    with open(ACTIVE_OUTPUT_DIR / "dataset.exps", "w", encoding="utf-8") as f_exps:
        for face_name, shape_name, color_name in tqdm(triplets, desc="Build v22 SATD cache"):
            ...
            with Image.open(face_path) as face_img:
                face_tensor = T.functional.to_tensor(face_img.convert("RGB"))
            with Image.open(shape_path) as shape_img:
                shape_tensor = T.functional.to_tensor(shape_img.convert("RGB"))
            with Image.open(color_path) as color_img:
                color_tensor = T.functional.to_tensor(color_img.convert("RGB"))
            ...
            color_mask_dilate, color_mask_erode = dilate_erosion.mask(color_mask)
            ...
            i_satd_256 = hair_fast.blend.downsample_256(i_satd)
            lock_mask = build_v22_lock_mask(align_shape, size=i_satd_256.shape[-2:])
            satd_primary_mask, _ = filter_parsing_to_primary_subject(get_segmentation(((i_satd + 1.0) * 0.5).clamp(0, 1)))
            satd_hair_mask = torch.where(
                satd_primary_mask == 13,
                torch.ones_like(satd_primary_mask),
                torch.zeros_like(satd_primary_mask),
            ).float()
            target_hair_mask = (satd_hair_mask + align_shape["HM_X"]).clamp(0, 1)

            save_latents(
                ACTIVE_OUTPUT_DIR,
                "V22",
                cache_name(face_name, shape_name, color_name),
                satd_256=((i_satd_256[0] + 1) / 2).clamp(0, 1),
                color_256=((name_to_embed["color"]["image_norm_256"][0] + 1.0) * 0.5).clamp(0, 1),
                target_hair_mask=target_hair_mask[0].float().clamp(0, 1),
                align_hair_mask=align_shape["HM_X"][0].float().clamp(0, 1),
                satd_hair_mask=satd_hair_mask[0].float().clamp(0, 1),
                reference_hair_mask=color_mask_erode[0].float().clamp(0, 1),
                lock_mask=lock_mask[0].float().clamp(0, 1),
                color_hair_mask=color_mask_dilate[0].float().clamp(0, 1),
            )
            if USER_SAVE_PANELS:
                panel_name = f"{face_name}__{shape_name}__{color_name}.png"
                save_panel(
                    ACTIVE_OUTPUT_DIR / "panels" / panel_name,
                    face_path,
                    shape_path,
                    color_path,
    ...
    print(f"satd cache dir: {satd_dir}")
