...
USER_BLENDING_CHECKPOINT = Path("output/blending_train_v8_small/checkpoints/best.pth")
...
USER_BLEND_COLOR_STRENGTH_OVERRIDE = 0.0  # 0 uses checkpoint metadata.
...
class BlendingStageOnlyV8:
    def __init__(self, args):
        ...
        print(
            f"[blending_infer_v8] loaded blending stage checkpoint tensors: {len(loaded)} "
    ...
    def blend_images(self, align_shape, align_color, name_to_embed) -> torch.Tensor:
        ...
        blend_s = torch.cat((face_s[:, :6], blend_tail), dim=1)
        ...
        blend_image_01 = ((blend_image + 1) / 2).clamp(0, 1)
        return blend_image_01[0]
...
def make_run_records(
    ...
    for index, (triplet, output_file) in enumerate(zip(triplets, outputs), start=1):
        record = {
            ...
            "blend_color_strength_override": USER_BLEND_COLOR_STRENGTH_OVERRIDE,
