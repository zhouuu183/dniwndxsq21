...
USER_OUTPUT_DIR = Path("output/blending_infer_v54")

USER_TRIPLET_MANIFEST = Path("input/blending_infer_triplets_v54/triplets.jsonl")
USER_REBUILD_TRIPLET_MANIFEST = False

USER_BLENDING_CHECKPOINT = Path("output/blending_train_v8/checkpoints/best.pth")
USER_PP_V54_CKPT = Path("output/pp_v54_checkpoints_small/last.pth")
USER_ENABLE_CLEANUP_FACE_REFINER = False
USER_EAR_LOW_ALPHA = 0.15
USER_EAR_MASK_INIT_BIAS = -2.2
...
from hair_swap_v54 import HairFastV54, get_parser
...
def validate_checkpoints() -> None:
    for path, label in (
        ...
        (Path(USER_PP_V54_CKPT), "USER_PP_V54_CKPT"),
    ):
        if not path.exists():
...
def make_model_args():
    ...
    args.pp_checkpoint = str(USER_PP_V54_CKPT)
    args.pp_v54_checkpoint = str(USER_PP_V54_CKPT)
    ...
    args.earring_object_max_area_frac = USER_EARRING_OBJECT_MAX_AREA_FRAC
    args.earring_placement_dilate = USER_EARRING_PLACEMENT_DILATE
    args.earring_lobe_dilate = USER_EARRING_LOBE_DILATE
    args.earring_lobe_down_shift = USER_EARRING_LOBE_DOWN_SHIFT
    args.earring_outer_shift = USER_EARRING_OUTER_SHIFT
    args.earring_query_floor = USER_EARRING_QUERY_FLOOR
    args.earring_search_radius_y = USER_EARRING_SEARCH_RADIUS_Y
    args.earring_search_radius_x = USER_EARRING_SEARCH_RADIUS_X
    args.earring_search_attach_y_ratio = USER_EARRING_SEARCH_ATTACH_Y_RATIO
    args.earring_align_to_target = bool(USER_EARRING_ALIGN_TO_TARGET)
    args.earring_align_strength = USER_EARRING_ALIGN_STRENGTH
    args.earring_align_max_shift = USER_EARRING_ALIGN_MAX_SHIFT
    args.earring_attach_y_ratio = USER_EARRING_ATTACH_Y_RATIO
    args.earring_guarded_composite = bool(USER_EARRING_GUARDED_COMPOSITE)
    args.earring_composite_strength = USER_EARRING_COMPOSITE_STRENGTH
    args.earring_composite_feather = USER_EARRING_COMPOSITE_FEATHER
    args.earring_composite_sigma = USER_EARRING_COMPOSITE_SIGMA
    args.earring_composite_strict_mask = bool(USER_EARRING_COMPOSITE_STRICT_MASK)
    args.earring_composite_visible_gate_strength = USER_EARRING_COMPOSITE_VISIBLE_GATE_STRENGTH
    args.earring_composite_target_hair_gate_strength = USER_EARRING_COMPOSITE_TARGET_HAIR_GATE_STRENGTH
    args.earring_composite_hair_block_gate_strength = USER_EARRING_COMPOSITE_HAIR_BLOCK_GATE_STRENGTH
    args.earring_composite_alpha_floor = USER_EARRING_COMPOSITE_ALPHA_FLOOR
    args.earring_composite_max_area_frac = USER_EARRING_COMPOSITE_MAX_AREA_FRAC
    args.earring_composite_placement_dilate = USER_EARRING_COMPOSITE_PLACEMENT_DILATE
    args.earring_composite_search_restrict_strength = USER_EARRING_COMPOSITE_SEARCH_RESTRICT_STRENGTH
...
def main() -> None:
    ...
    model = HairFastV54(make_model_args())
    ...
    for index, (triplet, output_file) in enumerate(tqdm(list(zip(triplets, output_files)), desc="Generate v54 images"), start=1):
