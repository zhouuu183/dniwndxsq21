...
class Config:
    ...
    batch_size = 8
    epochs = 120
    gradient_accumulation_steps =1   # 梯度累积步数，有效batch_size = batch_size * gradient_accumulation_steps
    ...
    use_amp = True                               # 是否使用混合精度训练（AMP）优化显存
    use_gradient_checkpointing = False            # 是否使用梯度检查点（进一步节省显存，但会减慢训练速度）
...
class Trainer:
    ...
    def train_one_epoch(self):
        ...
        for batch in tqdm(self.train_dataloader):
            ...
            # 根据配置决定是否使用AMP
            autocast_context = autocast() if self.config.use_amp else torch.no_grad().__enter__()
            
            with autocast_context:
                latent_s, latent_f = self.model(self.normalize(source), self.normalize(target), target_mask, HT_E)

            # StyleGAN2的generator不支持FP16，需要强制使用FP32
            with torch.cuda.amp.autocast(enabled=False):
                # 根据配置决定是否使用梯度检查点
                if self.config.use_gradient_checkpointing:
                    gen_im_W, _ = checkpoint(self.net.generator, [latent_s], 
                                           {'input_is_latent': True, 'return_latents': False}, 
                                           use_reentrant=False)
                    F_w, _ = checkpoint(self.net.generator, [latent_s], 
                                       {'input_is_latent': True, 'return_latents': False, 
                ...
                # 根据配置决定是否使用梯度检查点
                if self.config.use_gradient_checkpointing:
                    gen_im_F, _ = checkpoint(self.net.generator, [latent_s], 
                                           {'input_is_latent': True, 'return_latents': False,
            ...
            # 如果使用AMP，需要将source_1024转换为FP32以匹配gen_im_F
            if self.config.use_amp:
                source_1024 = source_1024.float()

            # 损失计算：如果使用AMP，generator输出是FP32，需要禁用AMP避免类型不匹配
            if self.config.use_amp:
                # 在AMP模式下，generator输出是FP32，损失计算也需要FP32
                ...
                losses['loss'] = sum(losses.values()) / self.config.gradient_accumulation_steps
            else:
                losses = self.LossBuilder(source, target, target_mask, HT_E, gen_im_W, F_w, gen_im_F, latent_f)
            ...
            # 释放中间变量
            del gen_im_W, F_w, gen_im_F, latent_s, latent_f, latent_f_gen
            ...
            if self.config.use_adv and self.cur_iter >= self.config.iter_before:
                ...
                # 判别器训练：如果使用AMP，需要确保类型一致
                if self.config.use_amp:
                    # gen_im_F是FP32，source_1024可能是FP16，统一为FP32
    ...
    def validate(self, epoch):
        ...
        for batch_idx, batch in enumerate(tqdm(self.test_dataloader)):
            ...
            # 释放中间变量
            del gen_im_W, F_w, gen_im_F, latent_s, latent_f
            torch.cuda.empty_cache()
