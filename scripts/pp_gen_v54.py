...
from hair_swap_v54 import HairFastV54 as HairFast, get_parser
from models.ear_modules_v54 import (
    EarAnchoredQueryBuilder,
    FaceParsingHelperV54,
    HairMaskExtractorV54,
...
USER_OUTPUT_DIR_SMALL = Path("images/pp_dataset_v54_dual_small")
...
USER_OUTPUT_DIR_FULL = Path("images/pp_dataset_v54_dual_full")
...
USER_EARRING_SEARCH_RADIUS_Y = 28
USER_EARRING_SEARCH_RADIUS_X = 18
...
def build_parser(defaults):
    parser = argparse.ArgumentParser(description="PP dataset generator v54")
...
class DatasetItemBatchBuilder:
    def __init__(self, args):
        ...
        self.hair_mask_extractor = HairMaskExtractorV54(device=self.device, dilate_erosion=args.smooth)
        self.parsing_helper = FaceParsingHelperV54(parse_size=args.ear_parse_size)
        query_builder_kwargs = {
            ...
            "earring_search_radius_y": args.earring_search_radius_y,
            "earring_search_radius_x": args.earring_search_radius_x,
            "earring_search_attach_y_ratio": args.earring_search_attach_y_ratio,
        }
    ...
    def iter_batches(self, experiments, dataset_path, face_gallery_root):
        ...
        for batch in batch_iterator:
            ...
            earring_locator_mask = locator_masks["earring_locator_mask"] * source_detection_roi
            locator_present = (earring_locator_mask.flatten(1).sum(dim=1) >= 3).view(-1, 1, 1, 1)
            source_object_mask = torch.where(locator_present, earring_locator_mask, source_object_mask)
            source_earring_mask = torch.clamp(
                earring_masks.get("source_earring_clean_mask", query_info["source_earring_mask"])
                + source_object_mask
                + earring_locator_mask
                + earring_masks["earring_confident_mask"]
                + earring_masks["earring_highlight_mask"]
                + query_info["source_earring_mask"] * source_detection_roi,
                0,
                1,
            )
            source_earring_mask = torch.clamp(source_earring_mask + source_object_mask + earring_locator_mask, 0, 1) * source_detection_roi
            source_earring_mask = torch.where(locator_present, earring_locator_mask, source_earring_mask)
            earring_confident_mask = torch.clamp(earring_masks["earring_confident_mask"] + earring_locator_mask, 0, 1)
            earring_confident_mask = torch.where(locator_present, earring_locator_mask, earring_confident_mask)
            earring_highlight_mask = torch.clamp(
                earring_masks["earring_highlight_mask"] + 0.35 * earring_locator_mask,
                0,
                1,
            )
            ...
            for idx in range(batch_size):
                item = {
                    ...
                    "earring_search_roi": query_info.get("earring_search_roi", query_info["ear_roi"])[idx].cpu(),
                    ...
                    "earring_confident_mask": earring_confident_mask[idx].cpu(),
                    "earring_highlight_mask": earring_highlight_mask[idx].cpu(),
                    "earring_locator_mask": earring_locator_mask[idx].cpu(),
                }
                for key in CLEANUP_MASK_KEYS:
            ...
            del source_earring_mask, earring_locator_mask, earring_confident_mask, earring_highlight_mask
