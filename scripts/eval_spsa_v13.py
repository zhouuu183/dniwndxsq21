...
from hair_swap_author_v13 import HairFastAuthorV13, get_author_parser_v13
...
USER_SAVE_ALL = False
USER_INCLUDE_AUTHOR_PREVIEW = True
# ========================================================================
...
def build_parser():
    ...
    parser.add_argument("--include_author_preview", type=int, default=int(USER_INCLUDE_AUTHOR_PREVIEW))
    return parser
...
def main(args):
    ...
    author_model = None
    if bool(args.include_author_preview):
        author_args = get_author_parser_v13().parse_args([])
        author_args.device = args.device
        author_args.save_all = False
        author_args.use_shadow_cleanup = False
        author_model = HairFastAuthorV13(author_args)
    ...
    for record in tqdm(records, desc="Eval fixed SPSA pairs"):
        ...
        if author_model is not None:
            author_baseline = author_model.swap(
                source_path,
                reference_path,
                reference_path,
