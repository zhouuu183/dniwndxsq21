"""Create/verify the frozen CelebA-HQ evaluation manifest for Stable-Hair.

This is the Stable-Hair companion to ``celeba_hq_make_eval_pairs_v5.py``.  The
sampling logic is byte-for-byte identical to that script (same seed 3407, same
deterministic derangement), so it produces the *same* manifest and the two are
interchangeable.  The manifest is the contract shared by every method in a
comparison; the generator script (``celeba_hq_generate_eval_stable_hair.py``)
never creates new pairs, it only consumes this file.

By default this script is READ-ONLY: it loads and validates the existing
frozen manifest and reports its SHA256.  Set ``USER_REBUILD_MANIFEST = True``
only to create it the first time (it must then match the HairFast manifest
exactly, or the comparison is invalid).

Stable-Hair is a two-input (source + one reference) model, so the default mode
is ``both``.  ``full`` triplets are also produced identically to v5 for any
3-input model that joins the same comparison, but Stable-Hair itself only runs
``both``.
"""
...
USER_MODE = "both"  # "both": source + one ref; "full": source + shape ref + color ref
...
def main() -> None:
    ...
    if manifest_path.exists() and not USER_REBUILD_MANIFEST:
        ...
        print(f"Reused existing frozen manifest: {manifest_path}")
        print(f"Mode: {mode}; rows: {len(rows)}; sha256: {file_digest(manifest_path)}")
        print(
            "This is the shared evaluation contract. Keep USER_REBUILD_MANIFEST=False; "
            "every method (HairFast, Stable-Hair, ...) must consume this exact file."
        )
        return

    if USER_REBUILD_MANIFEST:
        print(
            "WARNING: USER_REBUILD_MANIFEST=True will (over)write the frozen manifest. "
            "The result must be byte-identical to the HairFast manifest (same seed 3407) "
            "or the comparison is invalid. Re-set USER_REBUILD_MANIFEST=False afterwards."
        )
