...
from losses.pp_losses_v51 import EarAwareLossBuilder
...
USER_DATASET_DIR_SMALL = Path("images/pp_dataset_v51_dual_small")
USER_OUTPUT_DIR_SMALL = Path("output/pp_v51_checkpoints_small")
USER_RUN_NAME_SMALL = "ear_refine_v51_dual_small"

USER_DATASET_DIR_FULL = Path("images/pp_dataset_v51_dual_full")
USER_OUTPUT_DIR_FULL = Path("output/pp_v51_checkpoints_full")
USER_RUN_NAME_FULL = "ear_refine_v51_dual_full"
...
def build_parser(defaults):
    parser = argparse.ArgumentParser(description="Post Process trainer v51")
...
def configure_training_stage(model: PostProcessModelV51, args):
...
class TrainerV51:
    ...
    def save_validation_images(self, files, epoch_tag):
        ...
        with open(vis_dir / "columns.txt", "w", encoding="utf-8") as file:
            file.write("source | target | gen_f | source_earring | recall_mask | ear_detail_query | prior_mask | write_mask | fine_mask | injected_64 | direct_alpha\n")
    ...
    def _run_model(self, batch):
        ...
        HT_E = batch["HT_E"]
        source_ear_mask = (batch["source_earring_mask"] > 0.05).float()
...
class PPDatasetV51(Dataset):
    def __init__(self, dataset_index: DatasetPartIndexV51, sample_indices, is_test=False):
        super().__init__()
    ...
    def transform(self, sample):
        ...
        keys_to_flip = ["source", "target", "target_mask", "HT_E", "source_hair_mask", "target_hair_mask",
                        ...
                        "source_hair_block_mask", "source_earring_recall_mask", *CLEANUP_MASK_KEYS]
...
def build_dataset_index(dataset_dir: Path, query_area_threshold: float):
    ...
    return DatasetPartIndexV51(files, part_lengths, positive_hints)
...
def main(args):
    ...
    train_dataset = PPDatasetV51(dataset_index, train_indices)
    positive_train_indices = [idx for idx in train_indices if dataset_index.is_positive(int(idx))]
    train_dataset_positive = PPDatasetV51(dataset_index, positive_train_indices) if positive_train_indices else None
    test_dataset = PPDatasetV51(dataset_index, test_indices, is_test=True)
    ...
    logger = WandbLogger(name=args.name_run, project="HairFast-PostProcess-V51") if args.use_wandb else NullLoggerV51(
        args.checkpoint_dir
    )
    logger.start_logging()

    model = PostProcessModelV51(args)
    ...
    trainer = TrainerV51(model, args, optimizer, train_dataloader, train_dataloader_positive, test_dataloader, logger)
