...
from losses.pp_losses_v52 import EarAwareLossBuilder
from models.Net import Net
from models.postprocess_v52 import PostProcessModelV52
...
USER_DATASET_DIR_SMALL = Path("images/pp_dataset_v52_dual_small")
USER_OUTPUT_DIR_SMALL = Path("output/pp_v52_checkpoints_small")
USER_RUN_NAME_SMALL = "ear_refine_v52_dual_small"

USER_DATASET_DIR_FULL = Path("images/pp_dataset_v52_dual_full")
USER_OUTPUT_DIR_FULL = Path("output/pp_v52_checkpoints_full")
USER_RUN_NAME_FULL = "ear_refine_v52_dual_full"
...
USER_EARRING_QUERY_DILATE = 5
USER_EARRING_INJECTION_DILATE = 1
USER_EARRING_PRIOR_DILATE = 1
USER_EARRING_FINE_MASK_FLOOR = 0.75
USER_ENABLE_DIRECT_EARRING_RESTORE = True
USER_DIRECT_EARRING_RESTORE_STRENGTH = 0.95
USER_DIRECT_EARRING_RESTORE_FEATHER = 3
USER_DIRECT_EARRING_RESTORE_SIGMA = 1.0
...
USER_LAMBDA_CLEANUP_ANCHOR = 2.0
USER_LAMBDA_CLEANUP_NON_DARK = 2.0
USER_LAMBDA_DETAIL_HIGH = 1.0
USER_LAMBDA_DETAIL_LOW_ANCHOR = 0.8
...
USER_LAMBDA_EARRING_RECALL_RGB = 3.0
USER_LAMBDA_EARRING_RECALL_HIGH = 6.0
USER_LAMBDA_EARRING_RECALL_EDGE = 2.0
USER_LAMBDA_EARRING_RECALL_NO_HALO = 3.0
...
RESOLVED_USER_CONFIG = {
    ...
    "earring_injection_dilate": USER_EARRING_INJECTION_DILATE,
    "earring_prior_dilate": USER_EARRING_PRIOR_DILATE,
    "earring_fine_mask_floor": USER_EARRING_FINE_MASK_FLOOR,
    "enable_direct_earring_restore": USER_ENABLE_DIRECT_EARRING_RESTORE,
    "direct_earring_restore_strength": USER_DIRECT_EARRING_RESTORE_STRENGTH,
    "direct_earring_restore_feather": USER_DIRECT_EARRING_RESTORE_FEATHER,
    "direct_earring_restore_sigma": USER_DIRECT_EARRING_RESTORE_SIGMA,
    ...
    "earring_recall_rgb": USER_LAMBDA_EARRING_RECALL_RGB,
    "earring_recall_high": USER_LAMBDA_EARRING_RECALL_HIGH,
    "earring_recall_edge": USER_LAMBDA_EARRING_RECALL_EDGE,
    "earring_recall_no_halo": USER_LAMBDA_EARRING_RECALL_NO_HALO,
...
def build_parser(defaults):
    parser = argparse.ArgumentParser(description="Post Process trainer v52")
    ...
    parser.add_argument("--earring_injection_dilate", type=int, default=defaults["earring_injection_dilate"])
    parser.add_argument("--earring_prior_dilate", type=int, default=defaults["earring_prior_dilate"])
    parser.add_argument("--earring_fine_mask_floor", type=float, default=defaults["earring_fine_mask_floor"])
    parser.add_argument("--enable_direct_earring_restore", type=str2bool, default=defaults["enable_direct_earring_restore"])
    parser.add_argument("--direct_earring_restore_strength", type=float, default=defaults["direct_earring_restore_strength"])
    parser.add_argument("--direct_earring_restore_feather", type=int, default=defaults["direct_earring_restore_feather"])
    parser.add_argument("--direct_earring_restore_sigma", type=float, default=defaults["direct_earring_restore_sigma"])
    ...
    parser.add_argument("--earring_recall_rgb", type=float, default=defaults["earring_recall_rgb"])
    parser.add_argument("--earring_recall_high", type=float, default=defaults["earring_recall_high"])
    parser.add_argument("--earring_recall_edge", type=float, default=defaults["earring_recall_edge"])
    parser.add_argument("--earring_recall_no_halo", type=float, default=defaults["earring_recall_no_halo"])
...
def configure_training_stage(model: PostProcessModelV52, args):
...
class TrainerV52:
    def __init__(
        ...
        if args is not None:
            loss_weights = {
                ...
                "earring_recall_rgb": args.earring_recall_rgb,
                "earring_recall_high": args.earring_recall_high,
                "earring_recall_edge": args.earring_recall_edge,
                "earring_recall_no_halo": args.earring_recall_no_halo,
            }
    ...
    def save_validation_images(self, files, epoch_tag):
        ...
        with open(vis_dir / "columns.txt", "w", encoding="utf-8") as file:
            file.write("source | target | gen_f | source_earring | recall_mask | clean_mask | ear_detail_query | prior_mask | write_mask | fine_mask | injected_64 | direct_alpha\n")
    ...
    def _run_model(self, batch):
        ...
        HT_E = batch["HT_E"]
        parser_ear_mask = (batch["source_earring_mask"] > 0.05).float()
        if "source_earring_recall_mask" in batch:
            recall_mask = (batch["source_earring_recall_mask"] > 0.05).float()
        else:
            recall_mask = parser_ear_mask
        if "source_earring_clean_mask" in batch:
            clean_mask = (batch["source_earring_clean_mask"] > 0.05).float()
        else:
            clean_mask = recall_mask
        source_ear_mask = torch.clamp(parser_ear_mask + clean_mask, 0, 1)
        ...
        for key in CLEANUP_MASK_KEYS:
            aux[key] = batch[key]
        aux["dataset_source_earring_mask"] = batch["source_earring_mask"]
        aux["source_earring_recall_mask"] = recall_mask
        aux["source_earring_clean_mask"] = clean_mask
        ...
        for batch in tqdm(self.test_dataloader):
            ...
            for idx in range(source.size(0)):
                ...
                preview_row = [
                    ...
                    target[idx].cpu(),
                    gen_f_256[idx].cpu(),
                    self.mask_to_rgb(aux.get("dataset_source_earring_mask")[idx] if aux.get("dataset_source_earring_mask") is not None else None, size),
                    self.mask_to_rgb(aux.get("source_earring_recall_mask")[idx] if aux.get("source_earring_recall_mask") is not None else None, size),
                    self.mask_to_rgb(aux.get("source_earring_clean_mask")[idx] if aux.get("source_earring_clean_mask") is not None else None, size),
                    self.mask_to_rgb(
                        aux.get("ear_detail_query_mask")[idx]
                        if aux.get("ear_detail_query_mask") is not None
                        else aux.get("hair_safe_query_mask")[idx]
                        if aux.get("hair_safe_query_mask") is not None
                        else aux.get("query_mask")[idx],
                        size,
                    ),
                    self.mask_to_rgb(aux.get("earring_prior_mask")[idx] if aux.get("earring_prior_mask") is not None else None, size),
                    self.mask_to_rgb(aux.get("earring_write_mask")[idx] if aux.get("earring_write_mask") is not None else None, size),
                    self.mask_to_rgb(aux.get("fine_mask")[idx] if aux.get("fine_mask") is not None else None, size),
                    self.mask_to_rgb(aux.get("injected_fine_mask_64")[idx] if aux.get("injected_fine_mask_64") is not None else None, size),
                    self.mask_to_rgb(
                        aux.get("direct_earring_restore_alpha")[idx]
                        if aux.get("direct_earring_restore_alpha") is not None
                        else None,
                        size,
                    ),
                ]
...
class PPDatasetV52(Dataset):
    def __init__(self, dataset_index: DatasetPartIndexV52, sample_indices, is_test=False):
        super().__init__()
    ...
    def transform(self, sample):
        ...
        keys_to_flip = ["source", "target", "target_mask", "HT_E", "source_hair_mask", "target_hair_mask",
                        ...
                        "source_hair_block_mask", "source_earring_recall_mask", "source_earring_clean_mask", *CLEANUP_MASK_KEYS]
    ...
    def __getitem__(self, idx):
        ...
        sample = {
            ...
            "source_earring_recall_mask": item.get(
                "source_earring_recall_mask",
                item["source_earring_mask"],
            ).clone(),
            "source_earring_clean_mask": item.get(
                "source_earring_clean_mask",
                item.get("source_earring_recall_mask", item["source_earring_mask"]),
            ).clone(),
...
def build_dataset_index(dataset_dir: Path, query_area_threshold: float):
    ...
    return DatasetPartIndexV52(files, part_lengths, positive_hints)


def is_positive_hint(item, query_area_threshold: float) -> bool:
    recall_mask = item.get("source_earring_recall_mask", item["source_earring_mask"])
    parser_positive = bool(
        item["source_earring_mask"].sum().item() > 0
        or recall_mask.sum().item() > 0
...
def main(args):
    ...
    train_dataset = PPDatasetV52(dataset_index, train_indices)
    positive_train_indices = [idx for idx in train_indices if dataset_index.is_positive(int(idx))]
    train_dataset_positive = PPDatasetV52(dataset_index, positive_train_indices) if positive_train_indices else None
    test_dataset = PPDatasetV52(dataset_index, test_indices, is_test=True)
    ...
    logger = WandbLogger(name=args.name_run, project="HairFast-PostProcess-V52") if args.use_wandb else NullLoggerV52(
        args.checkpoint_dir
    )
    logger.start_logging()

    model = PostProcessModelV52(args)
    ...
    trainer = TrainerV52(model, args, optimizer, train_dataloader, train_dataloader_positive, test_dataloader, logger)
