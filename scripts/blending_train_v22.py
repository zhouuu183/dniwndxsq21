...
from models.ColorTransfer_v22 import V22ColorTransfer, masked_mean_std, rgb_to_lab


# ========================= User Config: edit here only =========================
USER_DATASET_PROFILE = "small"

USER_DATASET_DIR_FFHQ = Path("input/blending_dataset_v22")
USER_OUTPUT_DIR_FFHQ = Path("output/blending_train_v22")
USER_VAL_SIZE_FFHQ = 512

USER_DATASET_DIR_SMALL = Path("input/blending_dataset_v22_small")
USER_OUTPUT_DIR_SMALL = Path("output/blending_train_v22_small")
...
USER_V22_AB_STRENGTH = 0.58
USER_V22_AB_STD_STRENGTH = 0.10
USER_V22_LUMA_BOOST_SCALE = 0.20
USER_V22_MAX_LUMA_SHIFT = 5.0
USER_V22_MIN_LUMA_SHIFT = -3.0
USER_V22_SATURATION_COMPRESS = 0.92
USER_V22_EXTREME_AB_SCALE = 0.74
USER_V22_LUMA_GAP_THRESHOLD = 0.12
USER_V22_EXTREME_LUMA_GAP = 0.25
USER_V22_DARK_LUMA_THRESHOLD = 28.0
USER_V22_LIGHT_LUMA_THRESHOLD = 62.0
USER_V22_SHADOW_THRESHOLD = 18.0
USER_V22_HIGHLIGHT_THRESHOLD = 82.0
USER_V22_HIGHLIGHT_SHADOW_AB_SCALE = 0.42
USER_V22_BINS = 7
USER_V22_TARGET_ERODE = 1
USER_V22_LOCK_DILATE = 1
USER_V22_ALPHA_BLUR_RADIUS = 3
USER_V22_REF_ERODE = 2
USER_V22_REF_L_MIN = 5.0
USER_V22_REF_L_MAX = 95.0
USER_V22_REF_MIN_SAT = 0.02

USER_LAMBDA_AB_MEAN = 0.35
USER_LAMBDA_AB_STD = 0.15
USER_LAMBDA_AB_HIST = 0.00
USER_LAMBDA_LUMA_MEAN = 0.05
USER_LAMBDA_LUMA_KEEP = 2.00
USER_LAMBDA_LUMA_GRAD = 1.50
USER_LAMBDA_PARAM_REG = 0.50
...
def read_triplets(dataset_dir: Path) -> list[tuple[str, str, str]]:
    ...
    if not path.exists():
        raise FileNotFoundError(f"Cannot find {path}. Run scripts/blending_gen_v22.py first.")
    triplets = []
...
class V22Dataset(Dataset):
    ...
    def __getitem__(self, index: int):
        ...
        if not path.exists():
            raise FileNotFoundError(f"Missing v22 cache: {path}. Run scripts/blending_gen_v22.py again.")
        with np.load(path) as data:
            ...
            lock_mask = torch.from_numpy(data["lock_mask"]).float()
        return satd_256, color_256, target_hair_mask, reference_hair_mask, lock_mask


def v22_config() -> dict[str, object]:
    return {
        "ab_strength": USER_V22_AB_STRENGTH,
        "ab_std_strength": USER_V22_AB_STD_STRENGTH,
        "luma_boost_scale": USER_V22_LUMA_BOOST_SCALE,
        "max_luma_shift": USER_V22_MAX_LUMA_SHIFT,
        "min_luma_shift": USER_V22_MIN_LUMA_SHIFT,
        "saturation_compress": USER_V22_SATURATION_COMPRESS,
        "extreme_ab_scale": USER_V22_EXTREME_AB_SCALE,
        "luma_gap_threshold": USER_V22_LUMA_GAP_THRESHOLD,
        "extreme_luma_gap": USER_V22_EXTREME_LUMA_GAP,
        "dark_luma_threshold": USER_V22_DARK_LUMA_THRESHOLD,
        "light_luma_threshold": USER_V22_LIGHT_LUMA_THRESHOLD,
        "shadow_threshold": USER_V22_SHADOW_THRESHOLD,
        "highlight_threshold": USER_V22_HIGHLIGHT_THRESHOLD,
        "highlight_shadow_ab_scale": USER_V22_HIGHLIGHT_SHADOW_AB_SCALE,
        "bins": USER_V22_BINS,
        "target_erode": USER_V22_TARGET_ERODE,
        "lock_dilate": USER_V22_LOCK_DILATE,
        "alpha_blur_radius": USER_V22_ALPHA_BLUR_RADIUS,
        "ref_erode": USER_V22_REF_ERODE,
        "ref_l_min": USER_V22_REF_L_MIN,
        "ref_l_max": USER_V22_REF_L_MAX,
        "ref_min_sat": USER_V22_REF_MIN_SAT,
    }


def build_model(learnable: bool = True) -> V22ColorTransfer:
    return V22ColorTransfer(**v22_config(), learnable=learnable)


class V22Trainer:
    def __init__(self, model: V22ColorTransfer, train_loader: DataLoader, val_loader: DataLoader, device: torch.device):
    ...
    def clamp_parameters(self):
        ranges = {
            "ab_strength": (0.25, 0.75),
            "ab_std_strength": (0.00, 0.25),
            "luma_boost_scale": (0.00, 0.35),
            "max_luma_shift": (1.00, 7.0),
            "min_luma_shift": (-5.0, 0.0),
            "saturation_compress": (0.82, 1.00),
            "extreme_ab_scale": (0.45, 0.90),
            "highlight_shadow_ab_scale": (0.20, 0.70),
        }
        with torch.no_grad():
    ...
    def calc_loss(self, satd_256, color_256, target_hair_mask, reference_hair_mask, lock_mask):
        ...
        ref_l, ref_ab = ref_lab[:, 0:1], ref_lab[:, 1:3]
        satd_l = satd_lab[:, 0:1]
        ...
        ab_hist_loss = masked_hist_loss(out_ab, core, ref_ab, ref_mask, bins=12, value_min=-110.0, value_max=110.0)

        out_l_mean, _ = masked_mean_std(out_l, core)
        ref_l_mean, _ = masked_mean_std(ref_l, ref_mask)
        luma_mean_loss = F.l1_loss(out_l_mean, ref_l_mean)
        luma_keep_loss = masked_l1(out_l, satd_l, core)
        luma_grad_loss = masked_gradient_l1(out_l, satd_l, core)
        preserve_loss = masked_l1(output, satd_256, (1.0 - alpha).clamp(0, 1))
        ...
        total = (
            ...
            + USER_LAMBDA_LUMA_MEAN * luma_mean_loss
        ...
        torch.save(
            {
                ...
                "v22_state_dict": self.model.state_dict(),
    ...
    def load_resume_checkpoint(self) -> int:
        ...
        self.model.load_state_dict(checkpoint.get("v22_state_dict", checkpoint.get("model_state_dict", checkpoint)), strict=False)
    ...
    def train_one_epoch(self, epoch: int):
        ...
        total_loss = 0.0
        steps = 0
        progress = tqdm(self.train_loader, desc=f"V22 train {epoch + 1}/{USER_EPOCHS}", leave=False)
        for batch in progress:
            satd_256, color_256, target_hair_mask, reference_hair_mask, lock_mask = self.prepare_batch(batch)
    ...
    def validate(self, epoch: int):
        ...
        steps = 0
        preview_rows = []
        for batch in tqdm(self.val_loader, desc=f"V22 val {epoch + 1}/{USER_EPOCHS}", leave=False):
            satd_256, color_256, target_hair_mask, reference_hair_mask, lock_mask = self.prepare_batch(batch)
            loss, loss_info, output, aux = self.calc_loss(
                satd_256,
                color_256,
                target_hair_mask,
                reference_hair_mask,
                ...
                for idx in range(bsz):
                    preview_rows.append(
                        [
                            ...
                            color_256[idx : idx + 1],
                            output[idx : idx + 1],
                            mask_to_preview(aux["allowed"][idx : idx + 1]),
                            mask_to_preview(aux["alpha"][idx : idx + 1]),
                            mask_to_preview(aux["lock_guard"][idx : idx + 1]),
                        ]
                    )
                    if len(preview_rows) >= USER_LOG_IMAGE_COUNT:
                        break
        ...
        print(
            f"[v22] epoch={epoch + 1} val_loss={avg['loss']:.6f} "
            f"val_ab={avg['ab_mean']:.6f} val_hist={avg['ab_hist']:.6f} "
    ...
    def train_loop(self):
        ...
        for epoch in range(start_epoch, USER_EPOCHS):
            ...
            print(f"[v22] epoch={epoch + 1} train_loss={train_loss:.6f}")
...
def main():
    ...
    train_loader = DataLoader(
        V22Dataset(train_triplets, ACTIVE_DATASET_DIR),
    ...
    val_loader = DataLoader(
        V22Dataset(val_triplets, ACTIVE_DATASET_DIR),
        batch_size=USER_BATCH_SIZE,
        shuffle=False,
        num_workers=USER_NUM_WORKERS,
        pin_memory=USER_PIN_MEMORY and torch.cuda.is_available(),
    ...
    trainer = V22Trainer(model, train_loader, val_loader, device)
    print(
        f"[v22] dataset={ACTIVE_DATASET_DIR} batch_size={USER_BATCH_SIZE} lr={USER_LR} "
