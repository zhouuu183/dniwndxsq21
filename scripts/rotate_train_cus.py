...
class Trainer:
    def __init__(self,
        ...
        # init landmarks
        config_landmarks = utility.get_config(utility.landmarks_arg)
        self.kp_extractor = utility.get_net(config_landmarks)
        ...
        # 保存图片相关参数
        self.save_val_images_freq = config.get('save_val_images_freq', 5)
        self.val_images_dir = config.get('val_images_dir', 'files')
    ...
    def validate(self, epoch=None):
        ...
        # 根据频率保存验证图片到本地目录
        if epoch is not None and epoch % self.save_val_images_freq == 0:
            val_dir = os.path.join(self.val_images_dir, f'val_{epoch}')
            os.makedirs(val_dir, exist_ok=True)
            for i, idx in enumerate(idxs):
                grid = images_to_log[i]  # 已经计算好的PIL图像
                grid.save(os.path.join(val_dir, f'sample_{i}.jpg'))
            print(f"Saved validation images for epoch {epoch} to {val_dir}")
    ...
    def train_loop(self, epochs, start_epoch=0, resume=False):
        ...
        save_model_freq = self.config.get('save_model_freq', 10)  # 默认每10轮保存一次模型

        for epoch in range(start_epoch, epochs):
            self.train_one_epoch()
            loss = self.validate(epoch=epoch)  # 传入epoch用于保存图片

            # 保存last和checkpoint（每个epoch都保存）
            self.save_model('last', save_online=False)
            self.save_checkpoint(epoch, self.optimizer, self.best_loss)

            # 根据频率保存rotate_{epoch}.pth
            if epoch % save_model_freq == 0:
                model_path = f'rotate_{epoch}.pth'
...
def main():
    # 用户配置参数（替代命令行）
    config = {
        'name_run': 'cus_rotate_train',
        'dataset': './images/rotate/rotate_dataset.pkl',
        'use_hair_loss': True,           # 若需关闭，设为 False
        'batch_size': 8,
        'epochs': 150,
        'resume': False,
        'checkpoint_path': 'checkpoint.pth',
        'save_model_freq': 10,            # 每10轮保存一次 rotate_{epoch}.pth
        'save_val_images_freq':1,         # 每5轮保存一次验证图片到本地
        'val_images_dir': 'files',         # 验证图片保存根目录
    }

    seed_everything()
    data = list(torch.load(config['dataset']).values())
    ...
    test_dataloader = DataLoader(test_dataset, batch_size=config['batch_size'], pin_memory=True, shuffle=False,
                                 num_workers=2)

    logger = WandbLogger(name=config['name_run'], project='HairFast-Rotate')
