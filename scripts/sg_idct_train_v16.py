...
USER_CHROMA_STRENGTH_INIT_V16 = 0.90
USER_CHROMA_STD_STRENGTH_INIT_V16 = 0.0
USER_CHROMA_DELTA_LIMIT_INIT_V16 = 24.0
USER_TEXTURE_STRENGTH_INIT_V16 = 0.90
USER_LUMA_BINS_V16 = 9
USER_ALPHA_STRENGTH_INIT_V16 = 1.05
...
class SGIDCTTensorDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        h_transfer = data["H_transfer"] if "H_transfer" in data.files else data["H_align"]
        return {
            "I_satd": torch.from_numpy(data["I_satd"]).float(),
            "I_color": torch.from_numpy(data["I_color"]).float(),
            "H_align": torch.from_numpy(h_transfer).float(),
    ...
    for epoch in range(USER_EPOCHS):
        ...
        for batch in progress:
            ...
            with torch.no_grad():
                ...
                model.chroma_strength.clamp_(0.30, 1.35)
                model.chroma_std_strength.clamp_(0.0, 0.20)
                model.chroma_delta_limit.clamp_(10.0, 32.0)
                model.texture_strength.clamp_(0.20, 1.0)
                model.alpha_strength.clamp_(0.60, 1.25)
