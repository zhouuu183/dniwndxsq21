...
USER_GRAD_CLIP = 5.0
USER_BLEND_COLOR_STRENGTH = 3.0
USER_FACE_CLIP_LOSS_WEIGHT = 1.0
USER_HAIR_CLIP_LOSS_WEIGHT = 0.25
USER_HAIR_CHROMA_STATS_LOSS_WEIGHT = 12.0
USER_HAIR_CHROMA_HIST_LOSS_WEIGHT = 3.0
USER_HAIR_LUMA_STYLE_LOSS_WEIGHT = 0.40
...
class BlendingTrainerV8:
    ...
    @staticmethod
    def apply_blend_color_strength(base_tail: torch.Tensor, blend_tail: torch.Tensor) -> torch.Tensor:
        return base_tail + USER_BLEND_COLOR_STRENGTH * (blend_tail - base_tail)
    ...
    def calc_loss(
        ...
        face_keep_loss = self.masked_l1(i_gen, i_base, (face_keep_mask * satd_protect_mask).clamp(0, 1))
        ...
        torch.save(
            {
                ...
                "blend_color_strength": USER_BLEND_COLOR_STRENGTH,
    ...
    def load_resume_checkpoint(self) -> int:
        ...
        checkpoint_strength = float(checkpoint.get("blend_color_strength", 1.0))
        strength_mismatch = abs(checkpoint_strength - USER_BLEND_COLOR_STRENGTH) > 1e-6
        if strength_mismatch:
            print(
                f"[blending_v8] resume checkpoint blend_color_strength={checkpoint_strength:.3f}; "
                f"current USER_BLEND_COLOR_STRENGTH={USER_BLEND_COLOR_STRENGTH:.3f}; "
                "resetting best_loss tracking because the validation scale changed.",
                file=sys.stderr,
            )
        ...
        self.best_loss = float("inf") if strength_mismatch else float(checkpoint.get("best_loss", self.best_loss))
    ...
    def train_one_epoch(self, epoch: int):
        ...
        for batch in progress:
            ...
            blend_s_raw = self.model(align_s[:, 6:], color_s[:, 6:], face_i * target_mask, color_i * hm_3e)
            blend_s = self.apply_blend_color_strength(align_s[:, 6:], blend_s_raw)
...
def main():
    ...
    print(
        ...
        f"blend_color_strength={USER_BLEND_COLOR_STRENGTH} "
