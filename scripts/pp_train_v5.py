...
USER_EARRING_TARGET_EXCLUDE_DILATE = 9
USER_DATASET_EARRING_QUERY_DILATE = 5
USER_EARRING_VALID_ROI_DILATE = 9
USER_EARRING_VISIBLE_ROI_DILATE = 9
...
USER_OUTPUT_TARGET_HAIR_EAR_PRESERVE_DILATE = 3
...
USER_LAMBDA_EAR_LIGHTING = 0.10
USER_LAMBDA_EAR_HAIR_LEAK = 1.0
USER_LAMBDA_EAR_HAIR_ANCHOR = 5.0
USER_LAMBDA_EAR_COLOR = 3.0
USER_LAMBDA_EAR_HIGHLIGHT = 4.0
...
USER_TARGET_HAIR_EAR_ANCHOR_HAIR_DILATE = 3
...
USER_EARRING_WARMUP_EPOCHS = 0

USER_DATALOADER_START_METHOD = "auto"
USER_DATALOADER_PREFETCH_FACTOR = 2
USER_DATALOADER_PERSISTENT_WORKERS = True
USER_DATALOADER_PIN_MEMORY = True
# ============================================================================
...
RESOLVED_USER_CONFIG = {
    ...
    "earring_target_exclude_dilate": USER_EARRING_TARGET_EXCLUDE_DILATE,
    "dataset_earring_query_dilate": USER_DATASET_EARRING_QUERY_DILATE,
    "earring_valid_roi_dilate": USER_EARRING_VALID_ROI_DILATE,
    "earring_visible_roi_dilate": USER_EARRING_VISIBLE_ROI_DILATE,
    ...
    "output_target_hair_ear_preserve_dilate": USER_OUTPUT_TARGET_HAIR_EAR_PRESERVE_DILATE,
    ...
    "target_hair_ear_anchor_hair_dilate": USER_TARGET_HAIR_EAR_ANCHOR_HAIR_DILATE,
    ...
    "earring_warmup_epochs": USER_EARRING_WARMUP_EPOCHS,
...
def build_parser(defaults):
    ...
    parser.add_argument("--earring_target_exclude_dilate", type=int, default=defaults["earring_target_exclude_dilate"])
    parser.add_argument("--dataset_earring_query_dilate", type=int, default=defaults["dataset_earring_query_dilate"])
    parser.add_argument("--earring_valid_roi_dilate", type=int, default=defaults["earring_valid_roi_dilate"])
    parser.add_argument("--earring_visible_roi_dilate", type=int, default=defaults["earring_visible_roi_dilate"])
    ...
    parser.add_argument("--output_target_hair_ear_preserve_dilate", type=int, default=defaults["output_target_hair_ear_preserve_dilate"])
    ...
    parser.add_argument("--target_hair_ear_anchor_hair_dilate", type=int, default=defaults["target_hair_ear_anchor_hair_dilate"])
    ...
    parser.add_argument("--earring_warmup_epochs", type=int, default=defaults["earring_warmup_epochs"])
...
class TrainerV5:
    def __init__(
        ...
        if args is not None:
            loss_weights = {
                ...
                "earring_target_exclude_dilate": args.earring_target_exclude_dilate,
                "target_hair_ear_anchor": args.target_hair_ear_anchor,
                "target_hair_ear_source_reject": args.target_hair_ear_source_reject,
                "target_hair_ear_anchor_hair_dilate": args.target_hair_ear_anchor_hair_dilate,
            ...
            if args.training_stage == "joint_highres":
                loss_weights["ear_lighting"] = 0.0
                loss_weights["target_hair_ear_anchor"] = 0.0
                loss_weights["target_hair_ear_source_reject"] = 0.0
    ...
    def use_earring_warmup(self) -> bool:
        return (
            self.args is not None
            and self.args.training_stage == "joint_highres"
            and self.current_epoch < max(0, int(getattr(self.args, "earring_warmup_epochs", 0)))
        )

    @staticmethod
    def keep_earring_warmup_losses(losses):
        keep_keys = {
            "ear_mask",
            "ear_high",
            "ear_presence",
        ...
        filtered = {key: value for key, value in losses.items() if key in keep_keys}
        if filtered:
            return filtered

        first_value = next(iter(losses.values()))
        if torch.is_tensor(first_value):
            return {"ear_warmup_zero": first_value * 0.0}
        return {"ear_warmup_zero": 0.0}
    ...
    def save_validation_images(self, files, epoch_tag):
        ...
        for order, sample in enumerate(files[:preview_count]):
            image = image_grid(list(map(T.functional.to_pil_image, sample)), 1, len(sample))
    ...
    def _run_model(self, batch):
        ...
        if torch.is_tensor(highlight_gate) and earring_highlight_mask is not None:
            earring_highlight_mask = earring_highlight_mask * (
                highlight_gate.float().view(-1, 1, 1, 1) > 0.5
            ).float()
        source_ear_mask = (
        ...
        forward_earring_mask = None
        forward_earring_reference = None
        forward_earring_highlight = None
        if dataset_mask_gate is not None and "earring_confident_mask" in batch:
            forward_earring_mask = batch["earring_confident_mask"] * dataset_mask_gate.float()
            if "earring_reference" in batch:
                forward_earring_reference = batch["earring_reference"]
            if earring_highlight_mask is not None:
                forward_earring_highlight = earring_highlight_mask * dataset_mask_gate.float()
        ...
        latent_s, latent_f, aux = self.model(
            ...
            earring_supervision_mask=forward_earring_mask,
            earring_highlight_mask=forward_earring_highlight,
            earring_reference=forward_earring_reference,
        ...
        def merge_dataset_aux_mask(key, gate, *, override=False):
            if gate is None:
                aux[key] = batch[key]
            elif aux.get(key) is None or override:
                ...
                if key in ONLINE_EARRING_AUX_KEYS:
                    if aux.get(key) is None:
                        merge_dataset_aux_mask(
                            key,
                            resolve_dataset_gate(DATASET_AUX_MASK_FLAGS[key]),
                        )
                else:
                    aux[key] = batch[key]
        if "earring_search_mask" in batch:
            if aux.get("earring_search_mask") is None:
                merge_dataset_aux_mask(
                    "earring_search_mask",
                    resolve_dataset_gate(DATASET_AUX_MASK_FLAGS["earring_search_mask"]),
                )

        supervision_gate = resolve_dataset_gate(DATASET_AUX_MASK_FLAGS["earring_confident_mask"])
        if "earring_reference" in batch and supervision_gate is not None:
            aux_reference = aux.get("earring_reference", source)
            aux["earring_reference"] = torch.where(supervision_gate, batch["earring_reference"], aux_reference)
        if "earring_confident_mask" in batch and supervision_gate is not None:
            online_mask = aux.get("earring_confident_mask")
            if online_mask is None:
                online_mask = torch.zeros_like(batch["earring_confident_mask"])
            aux["earring_supervision_mask"] = torch.where(
                supervision_gate,
                batch["earring_confident_mask"],
                online_mask,
            )
    ...
    def train_one_epoch(self):
        ...
        for batch_idx, batch in enumerate(tqdm(loader)):
            ...
            if self.use_earring_warmup():
                losses = self.keep_earring_warmup_losses(losses)
    ...
    def validate(self, epoch_tag="initial"):
        ...
        def accumulate_mask_stat(name, value):
            if not torch.is_tensor(value):
                return
            mask = value.detach().float()
            if mask.ndim == 3:
                mask = mask.unsqueeze(1)
            if mask.ndim != 4:
                return
            flat = mask.flatten(1)
            entry = earring_stats.setdefault(name, {"ratio": 0.0, "nonempty": 0.0, "count": 0})
            entry["ratio"] += flat.mean(dim=1).sum().item()
            entry["nonempty"] += (flat.sum(dim=1) > 1.0).float().sum().item()
            entry["count"] += mask.size(0)
        ...
        for batch in tqdm(self.test_dataloader):
            ...
            accumulate_mask_stat("batch_earring_conf", batch.get("earring_confident_mask"))
            accumulate_mask_stat("aux_earring_supervision", aux.get("earring_supervision_mask"))
            accumulate_mask_stat("aux_earring_conf_before", aux.get("earring_confident_mask_before_target_hair_ear_protect"))
            accumulate_mask_stat("aux_earring_conf", aux.get("earring_confident_mask"))
            accumulate_mask_stat("dataset_query", aux.get("dataset_earring_query_mask"))
            accumulate_mask_stat("query_before", aux.get("query_mask_before_target_hair_ear_protect"))
            accumulate_mask_stat("query", aux.get("query_mask"))
            accumulate_mask_stat("target_covered_ear", aux.get("target_covered_ear_block_mask"))
            accumulate_mask_stat("target_hair_ear_block", aux.get("target_hair_ear_block_mask"))
            accumulate_mask_stat("fine_before", aux.get("fine_mask_before_target_hair_ear_protect"))
            accumulate_mask_stat("fine", aux.get("fine_mask"))
            accumulate_mask_stat("output_preserve", aux.get("output_target_preserve_mask"))
            ...
            for idx in range(source.size(0)):
                ...
                preview_row = [
                    source[idx].cpu(),
                    self.image_to_rgb(batch.get("reference_shape"), idx, image_size),
                    self.image_to_rgb(batch.get("reference_color"), idx, image_size),
                    target[idx].cpu(),
                    gen_w_256[idx].cpu(),
                    gen_f_256[idx].cpu(),
                ]
        ...
        if earring_stats:
            parts = []
            for key in sorted(earring_stats):
                entry = earring_stats[key]
                count = max(1, int(entry["count"]))
                parts.append(
                    f"{key}:ratio={entry['ratio'] / count:.6f},nonempty={entry['nonempty'] / count:.3f}"
                )
            print("[val earring stats] " + " | ".join(parts))
        ...
        if preview_files:
            ...
            images_to_log = [
                image_grid(list(map(T.functional.to_pil_image, sample)), 1, len(sample))
                for sample in preview_files[:preview_count]
            ]
...
class PPDatasetV5(Dataset):
    ...
    def load_optional_image(self, path, fallback):
        if not path:
    ...
    def transform(self, sample):
        ...
        keys_to_flip = ["source", "target", "earring_reference", "target_mask", "HT_E", "source_hair_mask", "target_hair_mask",
    ...
    def __getitem__(self, idx):
        ...
        if self.is_test:
            sample["reference_shape"] = self.load_optional_image(item.get("shape_path", ""), sample["target"])
            sample["reference_color"] = self.load_optional_image(item.get("color_path", ""), sample["target"])
        for key in CLEANUP_MASK_KEYS:
