...
#         'result_path': Path('output_v7/result_01.png'),
#     },
#     {
#         'face_dir': Path('input/faces'),
#         'shape_dir': Path('input/shapes'),
...
#         'result_path': Path('output_v7/result_02.png'),
#     },
# ]
USER_CONFIG = {
    'enabled': False,
...
def apply_user_config(args, model_args):
    ...
    model_args.smooth = int(USER_CONFIG.get('smooth', model_args.smooth))
    model_args.align_mask_size = int(USER_CONFIG.get('align_mask_size', model_args.align_mask_size))
    model_args.disable_post_process = bool(
        USER_CONFIG.get('disable_post_process', model_args.disable_post_process)
    )
    model_args.fringe_crop_size = int(USER_CONFIG.get('fringe_crop_size', model_args.fringe_crop_size))
    model_args.fringe_crop_padding = int(USER_CONFIG.get('fringe_crop_padding', model_args.fringe_crop_padding))
    model_args.fringe_mask_prior_weight = float(
        USER_CONFIG.get('fringe_mask_prior_weight', model_args.fringe_mask_prior_weight)
    )
...
if __name__ == "__main__":
    model_parser = get_parser_v7()
    parser = argparse.ArgumentParser(description='HairFast v7 evaluate')
    ...
    parser.add_argument('--output_dir', type=Path, default=Path('output_v7'), help='The directory for final results')
