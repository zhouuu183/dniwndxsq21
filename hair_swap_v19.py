...
from models.Blending_v19 import Blending_v19
...
class HairFast_v19:
    """
    HairFast v19 with boundary-and-layer guided irregular hairstyle transfer.
    """

    def __init__(self, args):
        self.args = args
        self.net = Net(self.args)
        self.embed = Embedding_v19(args, net=self.net)
        self.align = Alignment_v19(args, self.embed.get_e4e_embed, net=self.net)
        self.blend = Blending_v19(args, net=self.net)
...
def get_parser():
    parser = argparse.ArgumentParser(description="HairFast v19")

    parser.add_argument("--save_all_dir", type=Path, default=Path("output"), help="Directory for latent and debug outputs")
    ...
    parser.add_argument("--batch_size", type=int, default=3, help="Batch size for image encoding")
    parser.add_argument("--save_all", action="store_true", help="Save intermediate outputs")

    parser.add_argument("--mixing", type=float, default=0.95, help="Hair-region FS mixing during embedding")
    parser.add_argument("--smooth", type=int, default=5, help="Dilation / erosion radius")
    ...
    parser.add_argument("--use_shape_v19", action="store_true", default=True)
    parser.add_argument("--shape_adapter_v19_checkpoint", type=str, default="")
    parser.add_argument("--shape_adapter_v19_hidden", type=int, default=256)
    parser.add_argument("--shape_v19_use_rotation", action=argparse.BooleanOptionalAction, default=True)
    parser.add_argument("--shape_v19_strength", type=float, default=1.0)
    parser.add_argument("--shape_v19_prior_strength", type=float, default=0.35)
    parser.add_argument("--shape_v19_detail_strength", type=float, default=1.0)
    parser.add_argument("--shape_v19_final_detail_strength", type=float, default=0.65)
    parser.add_argument("--shape_v19_mask_residual_gain", type=float, default=1.0)
    parser.add_argument("--shape_v19_boundary_radius", type=int, default=3)
