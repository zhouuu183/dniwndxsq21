...
def enrich_delta_masks_with_halo(
    ...
    face_residue_seed = ((remove + 0.95 * remove_halo) * face_cleanup_surface).clamp(0, 1)
    remove_face = (
        _dilate(face_residue_seed, 1)
        * face_cleanup_surface
        * (1.0 - 0.42 * detail_protect)
    ).clamp(0, 1)
    remove_context = (remove_halo * context_region * (1.0 - 0.55 * _dilate(detail_protect, 2))).clamp(0, 1)
    neck_residue_seed = (
        (remove + 0.60 * remove_halo)
        * _dilate(neck_region, 5)
        * (1.0 - 0.62 * cloth_guard)
    ).clamp(0, 1)
    remove_neck = (_dilate(neck_residue_seed, 1) * (1.0 - 0.55 * cloth_guard)).clamp(0, 1)
    visible_body_anchor = (
    ...
    remove_tail = (
        (remove + 0.90 * remove_halo)
        * lower_region
        * (1.0 - 0.55 * face_region)
        * (1.0 - 0.62 * body_block)
    ).clamp(0, 1)
    body_reveal = (
...
def stack_cleanup_masks_v16(delta_masks: dict[str, torch.Tensor]) -> torch.Tensor:
