...
USER_DATASET_DIR_SMALL = Path("images/pp_dataset_v5_dual_ear_short_long")
...
# Problem 4B: revealed-forehead repair losses.  RE-ENABLED (were 0.0).  The base
# reconstruction loss excludes the revealed region (source-hair risk), so with
# these at 0 the revealed forehead had almost NO supervision -> the generator
# defaulted to smooth/airbrushed skin.  texture_stat pulls the revealed region's
# high-frequency energy (pore/texture amount) toward the source's visible skin so
# it stops being smooth; tone_continuity only matches the low-frequency channel
# mean (skin colour) to the adjacent recovered skin so no SATD band or
# covered/uncovered seam forms.  tone only touches colour, never texture, so it
# cannot re-flatten the forehead.
USER_LAMBDA_REVEALED_SKIN_TEXTURE = 4.0
USER_LAMBDA_REVEALED_SKIN_TONE = 5.0
