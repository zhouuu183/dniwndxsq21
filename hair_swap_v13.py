...
from models.Alignment_v13 import Alignment_v13
...
class HairFast_v13:
    def __init__(self, args):
        ...
        self.align = Alignment_v13(args, self.embed.get_e4e_embed, net=self.net)
        if getattr(args, "spsa_checkpoint", ""):
            self.align.load_spsa_checkpoint(args.spsa_checkpoint)
...
def get_parser_v13():
    parser = argparse.ArgumentParser(description="HairFast v13")
    ...
    parser.add_argument("--spsa_prior_dir", type=Path, default=None)
    parser.add_argument("--spsa_checkpoint", type=str, default="")
    parser.add_argument("--spsa_require_priors", action="store_true")
    parser.add_argument("--spsa_feature_channels", type=int, default=512)
    parser.add_argument("--spsa_hidden_channels", type=int, default=128)
    parser.add_argument("--spsa_bang_roi_size", type=int, default=8)
    parser.add_argument("--spsa_tail_roi_size", type=int, default=8)
    parser.add_argument("--spsa_alpha_init", type=float, default=0.2)
    parser.add_argument("--spsa_beta_bang_init", type=float, default=0.15)
    parser.add_argument("--spsa_beta_tail_init", type=float, default=0.15)
    return parser


if __name__ == "__main__":
    model_args = get_parser_v13()
    args = model_args.parse_args()
    hair_fast = HairFast_v13(args)
