...
FACE_PROTECT_LABELS = tuple(sorted(set(FACE_SURFACE_LABELS + RAW_DETAIL_LABELS)))
...
class EarAwareLossBuilder(LossBuilderMulti):
    ...
    def __call__(self, source, target, target_mask, HT_E, gen_w, F_w, gen_F, F_gen, aux=None, **kwargs):
        ...
        safe_mask = aux.get("earring_safe_mask")
        if safe_mask is not None:
            safe_mask = resize_mask(safe_mask, query_mask.shape[-2:])
        else:
            safe_mask = None
        core_mask = aux.get("earring_core_mask")
        if core_mask is not None:
            core_mask = resize_mask(core_mask, query_mask.shape[-2:])
        else:
            core_mask = None
        ...
        if safe_mask is not None:
            recall_mask = torch.clamp(
                safe_mask + 0.35 * (core_mask if core_mask is not None else 0),
                0,
                1,
            )
            reference_mask = recall_mask
            source_ear_mask = torch.clamp(source_ear_mask * dilate_mask(recall_mask, 5) + recall_mask, 0, 1)
        ...
        earring_dark_candidate_mask = aux.get("earring_dark_candidate_mask")
        if earring_dark_candidate_mask is not None:
            earring_dark_candidate_mask = resize_mask(earring_dark_candidate_mask, query_mask.shape[-2:])
        else:
            earring_dark_candidate_mask = torch.zeros_like(source_ear_mask)
        earring_injection_seed_mask = aux.get("earring_injection_seed_mask")
        if earring_injection_seed_mask is not None:
            earring_injection_seed_mask = resize_mask(earring_injection_seed_mask, query_mask.shape[-2:])
        else:
            earring_injection_seed_mask = torch.zeros_like(source_ear_mask)
        earring_composite_seed_mask = aux.get("earring_composite_seed_mask")
        if earring_composite_seed_mask is not None:
            earring_composite_seed_mask = resize_mask(earring_composite_seed_mask, query_mask.shape[-2:])
        else:
            earring_composite_seed_mask = torch.zeros_like(source_ear_mask)
        trusted_earring_seed = torch.clamp(earring_injection_seed_mask + earring_composite_seed_mask, 0, 1)

        if safe_mask is not None:
            support_base = torch.clamp(safe_mask + 0.35 * (core_mask if core_mask is not None else 0), 0, 1)
            support = dilate_mask(support_base, 5)
            earring_object_mask = earring_object_mask * support
            earring_confident_mask = torch.clamp(
                support_base + earring_object_mask + earring_confident_mask * support,
                0,
                1,
            )
        else:
            earring_confident_mask = torch.clamp(
                earring_confident_mask + reference_mask + source_ear_mask + earring_object_mask,
                0,
                1,
            )
        earring_highlight_mask = torch.clamp(
            earring_highlight_mask * dilate_mask(earring_confident_mask, 3) + 0.35 * earring_confident_mask,
            0,
            1,
        )
        if trusted_earring_seed.flatten(1).sum().item() > 0:
            seed_support = dilate_mask(
                trusted_earring_seed,
                int(self.losses_dict.get("ear_loss_seed_dilate", 5)),
            )
            query_mask = query_mask * seed_support
            reference_mask = torch.clamp(reference_mask * seed_support + trusted_earring_seed, 0, 1)
            source_ear_mask = torch.clamp(source_ear_mask * seed_support + trusted_earring_seed, 0, 1)
            earring_confident_mask = torch.clamp(
                earring_confident_mask * seed_support + trusted_earring_seed,
                0,
                1,
            )
            earring_object_mask = earring_object_mask * seed_support
            earring_highlight_mask = torch.clamp(
                earring_highlight_mask * seed_support + 0.5 * trusted_earring_seed,
                0,
                1,
            )
            earring_dark_candidate_mask = earring_dark_candidate_mask * seed_support
        elif bool(self.losses_dict.get("ear_loss_require_trusted_seed", True)):
            query_mask = torch.zeros_like(query_mask)
            reference_mask = torch.zeros_like(reference_mask)
            source_ear_mask = torch.zeros_like(source_ear_mask)
            earring_confident_mask = torch.zeros_like(earring_confident_mask)
            earring_object_mask = torch.zeros_like(earring_object_mask)
            earring_highlight_mask = torch.zeros_like(earring_highlight_mask)
            earring_dark_candidate_mask = torch.zeros_like(earring_dark_candidate_mask)

        if visible_ear_roi is not None:
            visible_ear_roi = ensure_mask_4d(visible_ear_roi).float()
            transfer_roi = torch.clamp(
                visible_ear_roi + source_ear_mask + reference_mask + earring_confident_mask + earring_object_mask,
                0,
                1,
            )
        ...
        source_ear_for_detail = resize_mask(source_ear_mask, gen_F_256_01.shape[-2:])
        ...
        face_guard_mask = parsing_label_mask(aux.get("target_parsing"), FACE_PROTECT_LABELS)
        if face_guard_mask is not None:
            face_guard_mask = resize_mask(face_guard_mask, gen_F_256_01.shape[-2:])
            source_face_guard = parsing_label_mask(aux.get("source_parsing"), FACE_PROTECT_LABELS)
            if source_face_guard is not None:
                face_guard_mask = torch.clamp(
                    face_guard_mask + resize_mask(source_face_guard, gen_F_256_01.shape[-2:]),
                    0,
                    1,
                )

            for reject_mask in (aux.get("source_hair_mask"), aux.get("target_hair_mask"), source_hair_block_mask):
                if reject_mask is not None:
                    face_guard_mask = face_guard_mask * (
                        1 - resize_mask(reject_mask, gen_F_256_01.shape[-2:])
                    ).clamp(0, 1)

            ear_guard_exclude = trusted_earring_seed
            for ear_key in (
                "target_earring_mask",
                ...
                ear_mask = aux.get(ear_key)
                if ear_mask is not None:
                    ear_guard_exclude = torch.clamp(
                        ear_guard_exclude + resize_mask(ear_mask, trusted_earring_seed.shape[-2:]),
                        0,
                        1,
                    )
            exclude_dilate = int(self.losses_dict.get("face_guard_exclude_earring_dilate", 9))
            if exclude_dilate > 1:
                ear_guard_exclude = dilate_mask(ear_guard_exclude, exclude_dilate)
            face_guard_mask = face_guard_mask * (
                1 - resize_mask(ear_guard_exclude, gen_F_256_01.shape[-2:])
            ).clamp(0, 1)

        if face_guard_mask is not None and face_guard_mask.flatten(1).sum().item() > 0:
            face_guard_low_weight = self.losses_dict.get("face_guard_low", 0.0)
            if face_guard_low_weight > 0:
                losses["face_guard_low"] = face_guard_low_weight * (
                    masked_l1(gen_F_256_01, target, face_guard_mask)
                    + 0.5 * masked_l1(low_pass_filter(gen_F_256_01), low_pass_filter(target), face_guard_mask)
                )

            face_guard_non_dark_weight = self.losses_dict.get("face_guard_non_dark", 0.0)
            if face_guard_non_dark_weight > 0:
                losses["face_guard_non_dark"] = face_guard_non_dark_weight * masked_non_darker(
                    rgb_to_gray(gen_F_256_01),
                    rgb_to_gray(target),
                    face_guard_mask,
                )

            face_guard_high_weight = self.losses_dict.get("face_guard_high", 0.0)
            if face_guard_high_weight > 0:
                losses["face_guard_high"] = face_guard_high_weight * masked_l1(
        ...
        earring_supervision = torch.clamp(
            dilate_mask(earring_confident_mask, earring_supervision_dilate) + earring_object_mask,
            0,
            1,
        )
        weak_pseudo_mask, _ = build_weak_ear_pseudo_mask(source_for_earring, query_mask, earring_confident_mask)
        mask_target = torch.clamp(weak_pseudo_mask + earring_supervision + earring_object_mask, 0, 1)
        query_expand = float(self.losses_dict.get("ear_query_expand", 0.05))
        supervision_mask = torch.clamp(
            mask_target + query_expand * query_mask + 0.5 * earring_highlight_mask + 0.35 * earring_object_mask,
            0,
            1,
        )
        ...
        direct_weight = self.losses_dict.get("earring_direct", 0.0)
        if direct_weight > 0:
            direct_mask = torch.clamp(
                trusted_earring_seed
                + reference_mask * dilate_mask(trusted_earring_seed, 3)
                + earring_highlight_mask
                + earring_dark_candidate_mask * dilate_mask(trusted_earring_seed, 3),
                0,
                1,
            )
            direct_mask = direct_mask * dilate_mask(trusted_earring_seed, 5)
            if direct_mask.flatten(1).sum().item() > 0:
                losses["earring_direct"] = direct_weight * (
                    masked_l1(gen_F_256_01, source_for_earring, direct_mask)
                    + 0.75
                    * masked_l1(
                        high_pass_filter(gen_F_256_01),
                        high_pass_filter(source_for_earring),
