...
class EarAwareLossBuilder(LossBuilderMulti):
    ...
    def __call__(self, source, target, target_mask, HT_E, gen_w, F_w, gen_F, F_gen, aux=None, **kwargs):
        ...
        clean_mask = aux.get("source_earring_clean_mask")
        ...
        if clean_mask is not None:
            clean_mask = (ensure_mask_4d(clean_mask).float() > 0.05).float()
            clean_mask = resize_mask(clean_mask, query_mask.shape[-2:])
            if visible_ear_roi is not None:
                clean_mask = clean_mask * visible_ear_roi
            source_ear_mask = torch.clamp(source_ear_mask + clean_mask, 0, 1)
            clean_mask_for_loss = resize_mask(clean_mask, gen_F_256_01.shape[-2:])
        if clean_mask_for_loss is None or clean_mask_for_loss.sum().item() <= 0:
            clean_mask_for_loss = recall_mask_for_loss

        if source_hair_block_mask is not None:
            source_hair_block_mask = resize_mask(source_hair_block_mask, query_mask.shape[-2:])
            earring_leak_exempt_mask = clean_mask if clean_mask is not None else recall_mask
            if earring_leak_exempt_mask is not None:
                source_hair_block_mask = source_hair_block_mask * (1 - dilate_mask(earring_leak_exempt_mask, 3)).clamp(0, 1)
        ...
        if clean_mask_for_loss is not None and clean_mask_for_loss.sum().item() > 0:
            ...
            if no_halo_weight > 0:
                no_halo_mask = (dilate_mask(clean_mask_for_loss, 11) - dilate_mask(clean_mask_for_loss, 3)).clamp(0, 1)
