...
# ========================= User Config Area: edit only here =========================
USER_CONFIG = {
    "enabled": False,
    "experiments": [
        {
            ...
            "shape_name": "7.png",
            "color_name": "7.png",
            "result_path": Path("output/result_v14.png"),
        },
    ],
    "save_all": True,
    "save_all_dir": Path("output/v14_debug"),
    "benchmark": False,
}
# ================================================================================
...
def main(model_args, args):
    hair_fast = HairFastV14(model_args)

    experiments = []
    configured_outputs = []
    if args.file_path is not None:
        with open(args.file_path, "r", encoding="utf-8") as handle:
            experiments.extend(handle.readlines())
    ...
    for exp, configured_output in tqdm(list(zip(experiments, configured_outputs))):
        ...
        result = hair_fast.swap(face_path, shape_path, color_path, benchmark=args.benchmark, exp_name=exp_name)
...
if __name__ == "__main__":
    model_parser = get_parser_v14()
    parser = argparse.ArgumentParser(description="HairFast v14 evaluate")
    ...
    parser.add_argument("--output_dir", type=Path, default=Path("output/v14"))
